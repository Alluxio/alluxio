...
import alluxio.dora.dora.stress.master.MultiOperationMasterBenchParameters;
import alluxio.dora.dora.stress.master.MultiOperationMasterBenchTaskResult;
...
public class MultiOperationStressMasterBench
  ...
  public void validateParams() throws Exception {
    ...
    if (numOperations < 2) {
      throw new InvalidArgumentException(String.format(
          "%s operations must contain at least two operations.", MultiOperationMasterBenchParameters.OPERATIONS_OPTION_NAME));
    }
    ...
    if (mParameters.mBasePaths.length != mParameters.mOperations.length) {
      throw new InvalidArgumentException(
          String.format("%s must contain the same number of params as %s .",
              MultiOperationMasterBenchParameters.BASES_OPTION_NAME, MultiOperationMasterBenchParameters.OPERATIONS_OPTION_NAME));
    }
    ...
    InvalidArgumentException duplicatedModeSpecifiedException = new InvalidArgumentException(
        String.format("Exact one of the following params %s, %s, %s must be specified",
            MultiOperationMasterBenchParameters.TARGET_THROUGHPUTS_OPTION_NAME, MultiOperationMasterBenchParameters.THREADS_RATIO_OPTION_NAME,
            MultiOperationMasterBenchParameters.OPERATIONS_RATIO_OPTION_NAME));
      ...
      if (mParameters.mTargetThroughputs.length != numOperations) {
        throw new InvalidArgumentException(
            String.format("If specified, %s must contain the same number of args as %s.",
                MultiOperationMasterBenchParameters.TARGET_THROUGHPUTS_OPTION_NAME, MultiOperationMasterBenchParameters.OPERATIONS_OPTION_NAME));
      }
      modeSpecified = true;
    }
    ...
    if (mParameters.mThreadsRatio != null) {
      if (mParameters.mThreadsRatio.length != numOperations) {
        throw new InvalidArgumentException(
            String.format("If specified, %s must contain the same number of args as %s.",
                MultiOperationMasterBenchParameters.THREADS_RATIO_OPTION_NAME, MultiOperationMasterBenchParameters.OPERATIONS_OPTION_NAME));
      }
      if (modeSpecified) {
    ...
    if (mParameters.mOperationsRatio != null) {
      if (mParameters.mOperationsRatio.length != numOperations) {
        throw new InvalidArgumentException(
            String.format("If specified, %s must contain the same number of args as %s.",
                MultiOperationMasterBenchParameters.OPERATIONS_RATIO_OPTION_NAME, MultiOperationMasterBenchParameters.OPERATIONS_OPTION_NAME));
      }
      if (modeSpecified) {
        throw duplicatedModeSpecifiedException;
      }
  ...
  private final class AlluxioNativeBenchThread extends BenchThread {
    ...
    private AlluxioNativeBenchThread(
            ...
            alluxio.dora.dora.client.file.FileSystem fs, int threadIndex) {
