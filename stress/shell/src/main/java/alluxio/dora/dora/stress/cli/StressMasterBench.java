...
import alluxio.dora.dora.stress.master.MasterBenchParameters;
import alluxio.dora.dora.stress.master.MasterBenchTaskResult;
...
import alluxio.dora.dora.hadoop.HadoopConfigurationUtils;
...
public class StressMasterBench extends StressMasterBenchBase<MasterBenchTaskResult,
  ...
  public void prepare() throws Exception {
    ...
    if (mParameters.mClientType == FileSystemClientType.ALLUXIO_HDFS) {
      ...
      InstancedConfiguration alluxioProperties = alluxio.dora.dora.conf.Configuration.copyGlobal();
      alluxioProperties.merge(HadoopConfigurationUtils.getConfigurationFromHadoop(hdfsConf),
          Source.RUNTIME);

      mCachedNativeFs = new alluxio.dora.dora.client.file.FileSystem[mParameters.mClients];
      for (int i = 0; i < mCachedNativeFs.length; i++) {
        mCachedNativeFs[i] = alluxio.dora.dora.client.file.FileSystem.Factory
  ...
  private final class AlluxioNativeBenchThread extends BenchThread {
    private final alluxio.dora.dora.client.file.FileSystem mFs;

    private AlluxioNativeBenchThread(BenchContext context, alluxio.dora.dora.client.file.FileSystem fs) {
      super(context);
      mFs = fs;
    }
