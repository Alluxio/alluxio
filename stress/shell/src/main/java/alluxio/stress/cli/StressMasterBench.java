...
public class StressMasterBench extends StressMasterBenchBase<MasterBenchTaskResult,
  ...
  @Override
  protected BenchContext<MasterBenchTaskResult> getContext() {
    RateLimiter rateLimiter = RateLimiter.create(mParameters.mTargetThroughput);
    return new BenchContext<MasterBenchTaskResult>(
        rateLimiter, mParameters.mOperation, mParameters.mBasePath);
  }

  @SuppressFBWarnings("BC_UNCONFIRMED_CAST")
  @Override
  protected BenchThread getBenchThread(
      BenchContext<MasterBenchTaskResult> context, int index) {
  ...
  protected abstract class BenchThread implements Callable<Void> {
    private final BenchContext<MasterBenchTaskResult> mContext;
    ...
    private BenchThread(BenchContext<MasterBenchTaskResult> context) {
      mContext = context;
      mResponseTimeNs = new Histogram(StressConstants.TIME_HISTOGRAM_MAX,
          StressConstants.TIME_HISTOGRAM_PRECISION);
      mBasePath = mContext.getBasePaths()[0];
      mFixedBasePath = mContext.getFixedBasePaths()[0];
    }
  ...
  private final class AlluxioHDFSBenchThread extends BenchThread {
    ...
    private AlluxioHDFSBenchThread(
        BenchContext<MasterBenchTaskResult> context, FileSystem fs) {
      super(context);
      mFs = fs;
    }
  ...
  private final class AlluxioNativeBenchThread extends BenchThread {
    ...
    private AlluxioNativeBenchThread(
        BenchContext<MasterBenchTaskResult> context,
        alluxio.client.file.FileSystem fs) {
      super(context);
      mFs = fs;
    }
    ...
    protected void applyOperation(long counter) throws IOException, AlluxioException {
      applyOperationNative(
          mFs, mParameters.mOperation, mBasePath, mFixedBasePath,
          counter, mParameters.mFixedCount);
    }
  }
  ...
  private final class AlluxioFuseBenchThread extends BenchThread {
    ...
    private AlluxioFuseBenchThread(BenchContext<MasterBenchTaskResult> context) {
      super(context);
