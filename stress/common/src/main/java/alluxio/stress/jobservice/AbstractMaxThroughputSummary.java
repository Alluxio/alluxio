...
public abstract class AbstractMaxThroughputSummary<T extends GeneralParameters, S extends Summary> implements Summary {
    ...
    @Override public alluxio.stress.GraphGenerator graphGenerator() {
    ...
    public static class GraphGenerator extends alluxio.stress.GraphGenerator {
        @Override public List<Graph> generate(List<? extends Summary> results) {
            List<Graph> graphs = new ArrayList<>();
            // only examine MaxThroughputSummary
            List<AbstractMaxThroughputSummary> summaries =
                results.stream().map(x -> (AbstractMaxThroughputSummary) x)
                    .collect(Collectors.toList());
            List<Enum> operations = summaries.stream().map(x -> x.mParameters.operation()).distinct().collect(Collectors.toList());
            // Iterate over all operations
            for (Enum operation : operations) {
                List<AbstractMaxThroughputSummary> opSummaries =
                    summaries.stream().filter(x -> x.mParameters.operation() == operation)
                        .collect(Collectors.toList());
                ...
                if (!opSummaries.isEmpty()) {
                    ...
                    for (AbstractMaxThroughputSummary summary : opSummaries) {
                        ...
                        subTitle.add(series + ": " + DateFormat.getDateTimeInstance()
                            .format(summary.getEndTimeMs()));
                    }

                    BarGraph maxGraph =
                        new BarGraph(operation + " - Max Throughput", subTitle, "Throughput");
                    ...
                    for (AbstractMaxThroughputSummary summary : opSummaries) {
                        ...
                        List<Summary> runs =
                            new ArrayList<>(summary.getPassedRuns().values());
                        alluxio.stress.GraphGenerator runGraphGenerator =
                            runs.get(0).graphGenerator();
