...
package alluxio.dora.dora.underfs.hdfs.acl;
...
import alluxio.dora.dora.underfs.hdfs.HdfsAclProvider;
...
public class SupportedHdfsAclProvider implements HdfsAclProvider {
  ...
  public Pair<AccessControlList, DefaultAccessControlList> getAcl(FileSystem hdfs, String path)
    ...
    for (AclEntry entry : hdfsAcl.getEntries()) {
      alluxio.dora.dora.security.authorization.AclEntry.Builder builder =
          new alluxio.dora.dora.security.authorization.AclEntry.Builder();
  ...
  public void setAclEntries(FileSystem hdfs, String path,
      List<alluxio.dora.dora.security.authorization.AclEntry> aclEntries) throws IOException {
    // convert AccessControlList into hdfsAcl
    List<AclEntry> aclSpecs = new ArrayList<>();

    for (alluxio.dora.dora.security.authorization.AclEntry entry : aclEntries) {
  ...
  private AclEntry getHdfsAclEntry(alluxio.dora.dora.security.authorization.AclEntry entry)
      throws IOException {
    AclEntry.Builder builder = new AclEntry.Builder();
    // Do not set name for unnamed entries
    if (entry.getType() != alluxio.dora.dora.security.authorization.AclEntryType.OWNING_USER
        && entry.getType() != alluxio.dora.dora.security.authorization.AclEntryType.OWNING_GROUP) {
   ...
   */
  private AclEntryType getHdfsAclEntryType(alluxio.dora.dora.security.authorization.AclEntry aclEntry)
      throws IOException {
   ...
   */
  private alluxio.dora.dora.security.authorization.AclEntryType getAclEntryType(AclEntry entry)
      throws IOException {
    switch (entry.getType()) {
      case USER:
        return entry.getName() == null || entry.getName().isEmpty()
            ? alluxio.dora.dora.security.authorization.AclEntryType.OWNING_USER
            : alluxio.dora.dora.security.authorization.AclEntryType.NAMED_USER;
      case GROUP:
        return entry.getName() == null || entry.getName().isEmpty()
            ? alluxio.dora.dora.security.authorization.AclEntryType.OWNING_GROUP
            : alluxio.dora.dora.security.authorization.AclEntryType.NAMED_GROUP;
      case MASK:
        return alluxio.dora.dora.security.authorization.AclEntryType.MASK;
      case OTHER:
        return alluxio.dora.dora.security.authorization.AclEntryType.OTHER;
      default:
