...
/**
 * A stream for writing a file into BOS. The data will be persisted to a temporary directory on the
 * local disk and copied as a complete file when the {@link #close()} method is called.
 */
@NotThreadSafe
public final class BOSOutputStream extends OutputStream {
  private static final Logger LOG = LoggerFactory.getLogger(BOSOutputStream.class);

  /** Bucket name of the Alluxio BOS bucket. */
  private final String mBucketName;
  /** Key of the file when it is uploaded to BOS. */
  ...
  /**
   * Creates a name instance of {@link BOSOutputStream}.
   *
   ...
   */
  public BOSOutputStream(String bucketName, String key, BosClient client, List<String> tmpDirs)
      throws IOException {
    ...
    Preconditions.checkArgument(key != null && !key.isEmpty(),
        "BOS path must not be null or empty.");
    Preconditions.checkArgument(client != null, "BosClient must not be null.");
   ...
   * uploaded to BOS Service. Once the file is uploaded, the temporary file is deleted.
   */
  @Override
  public void close() throws IOException {
    ...
    try (BufferedInputStream in = new BufferedInputStream(new FileInputStream(mFile))) {
      ...
      mBosClient.putObject(mBucketName, mKey, in, objMeta);
