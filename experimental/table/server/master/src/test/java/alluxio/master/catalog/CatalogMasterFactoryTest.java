...
public class CatalogMasterFactoryTest {
  ...
  @After
  public void after() {
    ServerConfiguration.global().set(PropertyKey.CATALOG_ENABLED, true);
  }
  ...
  public void enabled() throws Exception {
    ...
    assertTrue(names.contains(Constants.CATALOG_MASTER_NAME));
  }

  @Test
  public void disabled() throws Exception {
    ServerConfiguration.global().set(PropertyKey.CATALOG_ENABLED, false);
    MasterRegistry registry = new MasterRegistry();
    MasterUtils.createMasters(registry, mContext);
    Set<String> names =
        registry.getServers().stream().map(Server::getName).collect(Collectors.toSet());
    assertFalse(names.contains(Constants.CATALOG_MASTER_NAME));
  }
}
