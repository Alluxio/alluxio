...
public class UdbContext {
  ...
  public AlluxioURI getDbLocation() {
    ...
    return new AlluxioURI(
        PathUtils.concatPath(ServerConfiguration.get(PropertyKey.METADATA_PATH), mDbName, mType));
  }
  ...
  public AlluxioURI getTableLocation(String tableName) {
    return new AlluxioURI(PathUtils
        .concatPath(ServerConfiguration.get(PropertyKey.METADATA_PATH), mDbName, "tables",
            tableName, mType));
  }
}
