...
Tachyon, the tiers are ordered from top to bottom based on I/O performance. Therefore, the typical
...
Choosing which directory the data should placed is determined by the allocator, described later.
...
highest tier before reading from the worker. This can also be used as a data management strategy by
explicitly moving hot data to the highest tiers.
