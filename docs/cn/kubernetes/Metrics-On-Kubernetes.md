...
关于 Alluxio metrics 的基本信息，请参考 [Metrics System]({{ '/en/operation/Metrics-System.html' | relativize_url }}).
...
```shell
# 从默认 web 端口 20002 获取主 job master 组件的 metrics:
...
```shell
# 从默认 web 端口 49999 获取 fuse 组件的 metrics:
...
Alluxio 中需要开启 Prometheus 的 `PrometheusMetricsServlet`。为开启 Prometheus metrics sink，需要在 helm chart `value.yaml` 中配置如下参数：
```yaml
metrics:
  enabled: true
...
类似 HTTP JSON Sink，为访问 fuse web 的 metrics，我们需要配置 `alluxio.fuse.web.enabled` 为 true 以打开其 web 端口。

### 配置 Prometheus 客户端

使用 Prometheus 客户端从 Alluxio 中顺利获取 metrics, 需要配置 Prometheus 客户端的 `prometheus.yml`. 比如，获取 master metrics 时，需要如下配置:
...
获取其它组件的 metrics, 需要配置各自对应的 pod 角色标签和网络端口。
...
```shell
# 获取某具体组件的 Prometheus 格式的 metrics:
