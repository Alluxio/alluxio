...
用于检索用户。此类必须实现`alluxio.authentication.security.AuthenticationProvider`接口。
...
`alluxio.provider.group.security.ShellBasedUnixGroupsMapping`，该实现通过执行`groups` shell命令获取一个给定用户的组关系。
