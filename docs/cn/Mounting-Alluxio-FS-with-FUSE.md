...
Alluxio-FUSE是基于[FUSE]((http://fuse.sourceforge.net/))这个项目，并且都支持大多数的文件系统操作。但是由于Alluxio固有的属性，例如它的一次写不可改变的文件数据模型，该挂载的文件系统与POSIX标准不完全一致，尚有一定的局限性。因此，请先阅读本页面[局限性](#assumptions-and-limitations)这章，从而了解该特性的作用以及局限。
