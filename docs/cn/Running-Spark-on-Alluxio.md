...
* 我们建议您从Alluxio[下载页面](http://www.alluxio.org/download)下载tarball.
  另外，高级用户可以选择通过源代码编译客户端jar包，参考说明[这里](Building-Alluxio-Master-Branch.html#compute-framework-support)，并在这篇说明剩余部分将生成的jar包应用在配置项`{{site.ALLUXIO_CLIENT_JAR_PATH_BUILD}}`中。
