...
```properties
alluxio.coordinator.mount.table.root.ufs=s3n://S3_BUCKET/S3_DIRECTORY
```
...
1.(选项1):为根挂载设置
`alluxio-site.properties`中的`alluxio.coordinator.mount.table.root.shared=true`或
...
标记或为根挂载设置`alluxio.coordinator.mount.table.root.readonly=true`。
