...
嵌入式日记特性依赖于 [Copycat](https://github.com/atomix/copycat) 内置leader选举功能。内置leader选举功能不能与Zookeeper一起使用，因为系统不能接受两面可能选出不同leaders的状况。启用嵌入式日记就启用了Alluxio的内置leader election机制。请参阅[嵌入式日志配置文档]（{{'/en/operation/Journal.html'| relativize_url}}＃embedded-journal-configuration），以了解更多详细信息以及使用内部leader选举配置HA集群的替代方案。
...
1. 存放日志的共享存储系统（所有Alluxio masters均可访问）。leading master在此共享存储系统上写入日志，同时standby masters不断重播日志条目以保持与最新日志同步。建议将日志存储系统设置为：
  - 高度可用。所有master上元数据修改都需要写日志，因此日志存储系统的任何停机时间都将直接影响Alluxio master的可用性。
...
要验证Alluxio是否正在运行，访问lealdinger master的Web UI。要确定当前leading master，运行：
