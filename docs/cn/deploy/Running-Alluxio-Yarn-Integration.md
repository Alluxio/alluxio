...
注意：YARN 不是非常适合像 Alluxio 这样长时间运行的应用，我们推荐根据[该指南]({{ '/cn/deploy/Running-Alluxio-Yarn-Standalone.html' | relativize_url }})来将 Alluxio 与 YARN 一起运行，而不是作为 YARN 内的一个应用来运行。
