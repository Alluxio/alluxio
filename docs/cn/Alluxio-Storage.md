...
在Alluxio中有两种回收模式，异步和同步（默认）。你可以通过启用和禁用处理异步驱逐的空间预留器在这两者之间切换。
例如，要开启异步回收：
...
同步回收是初始默认的回收实施。它等待一个客户端请求比当前在worker上可用空间更多的空间，然后启动回收进程来释放足够的空间来满足这一要求。这导致了许多很小的回收尝试，使得效率较低但是使可用的Alluxio空间的利用最大化。在写或读缓存高工作负载时，异步回收可以提高性能。
