...
In preparation for using S3 with Alluxio, create a bucket (or use an existing bucket). You
...
Visit your S3 directory `S3_BUCKET/S3_DIRECTORY` to verify the files
...
avoided by setting the property `alluxio.underfs.s3a.inherit_acl=false`.
...
`alluxio-core-client-fs` module to use the [Alluxio file system interface]({{ '/en/api/FS-API.html' | relativize_url }}) or
...
To enable S3A streaming upload, you need to modify `conf/alluxio-site.properties` to include:
...
The S3A streaming upload feature addresses the above issues and is based on the
...
be tuned so that Alluxio worker uses a configuration optimized for the S3 backend.

```properties
# If the S3 connection is slow, a larger timeout is useful
alluxio.underfs.s3a.socket.timeout.ms=500sec
alluxio.underfs.s3a.request.timeout.ms=5min

# If we expect a high number of concurrent metadata operations
alluxio.underfs.s3.admin.threads.max=80
# If the toral metadata + data operations is high
alluxio.underfs.s3.threads.max=160

# If we expect a large number of concurrent writes to S3
