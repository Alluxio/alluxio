...
Specify the Azure SAS Token by adding the following property in `conf/alluxio-site.properties`:

```properties
alluxio.master.mount.table.root.option.fs.azure.sas.<AZURE_CONTAINER>.<AZURE_ACCOUNT>.blob.core.windows.net=<SAS Token>
```
...
```console
$ ./bin/alluxio fs mount \
  --option fs.azure.sas.<AZURE_CONTAINER>.<AZURE_ACCOUNT>.dfs.core.windows.net=<SAS Token> \
  /mnt/abfs abfs://<AZURE_CONTAINER>@<AZURE_ACCOUNT>.dfs.core.windows.net/<AZURE_DIRECTORY>/
# use abfss:// for HTTPS-only connections
```
