...
If the system status is `STRESSED` or `OVERLOADED`, Wa `WARN` level log would be printed containing the following the filesystem indicators:
```console
...
The monitoring indicators show the system status in a heuristic way, and that labels the system load in a certain extent.
