...
* A Fuse mount point. See [FUSE-based POSIX API]({{ '/en/fuse-sdk/Overview.html' | relativize_url }}) for more details on mounting.
...
* By using Fuse in worker process for reading, which needs to be configured in `${ALLUXIO_HOME}/conf/alluxio-site.properties`
  (see [FUSE-based POSIX API]({{ '/en/fuse-sdk/Overview.html' | relativize_url }})), the local read
