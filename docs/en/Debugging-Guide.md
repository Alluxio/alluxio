...
- If you are copying a file to Alluxio using `copyFromLocal` with `LocalFirstPolicy`,
the shell command will store data on the local worker (see [location policy](File-System-API.html#location-policy)).
In this case, you will see the above error once the local worker does not have enough space.

    Before version 1.6.0, `LocalFirstPolicy` is the default location policy for `copyFromLocal`.
