...
It checks the container READ/WRITE ACL to see if the Swift user or "*" is in the access control list. If necessary, you can customize the Swift container ACL checking in `SwiftUnderFileSystem.java`.
Swift user permission to the container is mapped to Alluxio permission to mounted directories and files.
