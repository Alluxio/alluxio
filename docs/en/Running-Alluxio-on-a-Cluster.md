...
are more workers than one. Instead you need to set up a shared storage to which all Alluxio servers
...
Finally, sync all the information to worker nodes. You can use
...
to sync the entire extracted `Alluxio` directory to workers. Besides, you can also use this command
to sync any change to `conf/alluxio-site.properties` to workers.
