...
Alluxio master service can be deployed as one primary master and several standby masters for fault tolerance. When the primary master goes down, a standby master is elected to become the new primary master.
...
This section describes the behavior of common read and write scenarios based on a typical Alluxio configuration as described above: Alluxio is colocated with the compute framework and and applications and the persistent storage system is either a remote storage cluster or cloud-based storage.

### Read

Sitting between the under storage and computation framework, Alluxio can serve as a caching layer for data reads. This subsection introduces different caching scenarios and their implications on performance.

#### Read Hits Alluxio Cache on a Local Worker
...
#### Read Hits Alluxio Cache on a Remote Worker
...
when Alluxio client reads a block that is not locally available, it will read and cache the block entirely on the local worker even if only part of the block is requested by the client. If there is no such a local worker, no cache will happen. For certain workloads and file formats (e.g., running SQL queries on ORC and Parquet files), the data reads are often not sequential. In this case, having partial caching will help users cache the input data into Alluxio even if only the footer or header of these files are read, significantly improving the performance of the following queries on the same input data.

In Alluxio versions 1.7 and above, partial caching is always enabled and handled on the server side in the background. The caching process is completely transparent to the client. The client will read data as normal and signal to the worker which blocks it reads asynchronously. The worker will fetch these blocks from the under storage or other Alluxio workers asynchronously. Duplicate requests caused by concurrent readers will be consolidated on the worker and result in caching the block once. Partial caching is not on the critical path, but may still impact performance if the network bandwidth between Alluxio and the under storage system is a bottleneck.
