...
cluster should run multiple Alluxio masters in [High Availability mode]({{ '/en/deploy/Install-Alluxio-Cluster-with-HA.html' | relativize_url }}).

There are two ways to enable HA mode in Alluxio, either with internal leader election and embedded journal, or external Zookeeper and a shared journal storage. It is recommended to use the second option for production use case.
...
You can find more on Embedded Journal configuration [here]({{ '/en/deploy/Install-Alluxio-Cluster-with-HA.html#raft-based-embedded-journal' | relativize_url }}).
...
You can find more on ZooKeeper and shared journal configuration [here]({{ '/en/deploy/Install-Alluxio-Cluster-with-HA.html#zookeeper-and-shared-journal-storage' | relativize_url }}).
...
You can find more details about the worker storage [here]({{ '/en/core-services/Data-Caching.html#configuring-alluxio-storage' | relativize_url }}).
...
- `--cap-add SYS_ADMIN` launches the container with [SYS_ADMIN](http://man7.org/linux/man-pages/man7/capabilities.7.html){:target="_blank"}
capability.
...
+ Option1: use [domain sockets](https://en.wikipedia.org/wiki/Unix_domain_socket){:target="_blank"} or
+ Option2: use [shared volumes](https://docs.docker.com/storage/volumes/){:target="_blank"}.
