...
named as `master.log`, `master.out`, `worker.log`, `worker.out` and `user_${USER}.log`. The file
with suffix `log` are generated by log4j, the file with suffix `out` are generated by redirect the
stdout and stderr.
