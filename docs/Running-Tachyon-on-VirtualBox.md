...
Command `run_vb.sh` allows you to launch a Tachyon on VirtualBox VMs. It reads Tachyon cluster configuration from `tachyon/deploy/vagrant/conf/init.yml`, then the script automatically creates the cluster, sets up under filesystem, and starts Tachyon master and workers on the cluster for you.
...
Provide your desired Tachyon cluster configuration. Edit `tachyon/deploy/vagrant/conf/init.yml`. The parameters in `tachyon/deploy/vagrant/conf/init.yml` are explained as the following.
