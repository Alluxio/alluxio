...
public final class TieredBlockStore implements BlockStore {
  ...
  public long lockBlock(long sessionId, long blockId) throws BlockDoesNotExistException {
    ...
    throw new BlockDoesNotExistException(
        ExceptionMessage.LOCK_RECORD_NOT_FOUND_FOR_BLOCK_AND_SESSION.getMessage(
                blockId, sessionId));
  }
  ...
  public TempBlockMeta createBlockMeta(long sessionId, long blockId, BlockStoreLocation location,
    ...
    throw new WorkerOutOfSpaceException(ExceptionMessage.NO_SPACE_FOR_BLOCK_ALLOCATION.getMessage(
  ...
  public void moveBlock(long sessionId, long blockId, BlockStoreLocation oldLocation,
    ...
    throw new WorkerOutOfSpaceException(ExceptionMessage.NO_SPACE_FOR_BLOCK_MOVE.getMessage(
  ...
  public void accessBlock(long sessionId, long blockId) throws BlockDoesNotExistException {
    ...
    mMetadataReadLock.unlock();
    if (!hasBlock) {
      throw new BlockDoesNotExistException(ExceptionMessage.NO_BLOCK_ID_FOUND.getMessage(blockId));
    }
    synchronized (mBlockStoreEventListeners) {
      for (BlockStoreEventListener listener : mBlockStoreEventListeners) {
  ...
  private void checkTempBlockOwnedBySession(long sessionId, long blockId)
    ...
    if (ownerSessionId != sessionId) {
      throw new InvalidWorkerStateException(
          ExceptionMessage.BLOCK_ID_FOR_DIFFERENT_SESSION.getMessage(blockId, ownerSessionId,
              sessionId));
    }
  }
    ...
    try {
      ...
      if (plan == null) {
        throw new WorkerOutOfSpaceException(
            ExceptionMessage.NO_EVICTION_PLAN_TO_FREE_SPACE.getMessage());
      }
    } finally {
      mMetadataReadLock.unlock();
    }
  ...
  private MoveBlockResult moveBlockInternal(long sessionId, long blockId,
    ...
    try {
      ...
      try {
        if (mMetaManager.hasTempBlockMeta(blockId)) {
          throw new InvalidWorkerStateException(
              ExceptionMessage.MOVE_UNCOMMITTED_BLOCK.getMessage(blockId));
        }
      ...
      if (!srcLocation.belongTo(oldLocation)) {
        throw new BlockDoesNotExistException(
            ExceptionMessage.BLOCK_NOT_FOUND_AT_LOCATION.getMessage(blockId, oldLocation));
      }
  ...
  private void removeBlockInternal(long sessionId, long blockId, BlockStoreLocation location)
    ...
    try {
      ...
      if (!blockMeta.getBlockLocation().belongTo(location)) {
        throw new BlockDoesNotExistException(
            ExceptionMessage.BLOCK_NOT_FOUND_AT_LOCATION.getMessage(blockId, location));
      }
      // Heavy IO is guarded by block lock but not metadata lock. This may throw IOException.
