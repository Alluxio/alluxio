...
func (p *WorkerProcess) InitCommandTree(processCmd *cobra.Command) {
	workerCmd := &cobra.Command{
		Use:   Worker.Name,
		Short: "Manages the Alluxio worker process",
	}
	processCmd.AddCommand(workerCmd)

	startCmd := env.NewProcessStartCmd(p)
	workerCmd.AddCommand(startCmd)

	workerCmd.AddCommand(env.NewProcessStopCmd(p))
}
...
func (p *WorkerProcess) Start() error {
