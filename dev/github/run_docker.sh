...
function main {
  ...
  if [ -n "${ALLUXIO_DOCKER_MVN_PROJECT_LIST_COMPILE}" ]; then
    run_args+=" -e ALLUXIO_MVN_PROJECT_LIST_COMPILE=${ALLUXIO_DOCKER_MVN_PROJECT_LIST_COMPILE}"
  fi

  if [ -n "${ALLUXIO_DOCKER_MVN_PROJECT_LIST_TEST}" ]; then
