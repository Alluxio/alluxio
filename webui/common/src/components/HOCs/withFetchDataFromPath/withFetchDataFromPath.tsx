...
export function withFetchDataFromPath<T extends IFetchDataFromPathProps>(
  ...
  > {
    ...
    private getParsedQuery(query: string): IRequest {
      const req: IRequest = parseQueryString(query);
      req.path = decodeURIComponent(req.path || '');
      req.offset = req.offset || '0';
      return req;
    }
  }
  (FetchDataFromPathHoc as React.ComponentType<
    any
  >).displayName = `withFetchDataFromPath(${getDisplayName(WrappedComponent)})`;
