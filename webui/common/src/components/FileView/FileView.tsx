...
export class FileView extends React.PureComponent<IFileViewProps> {
  public render(): JSX.Element {
    ...
    return (
      <React.Fragment>
        <h5>
          {viewData.currentDirectory
            ? viewData.currentDirectory.absolutePath
            : viewData.currentPath}
          : <small>First 5KB from {viewData.viewingOffset} in ASCII</small>
        </h5>
        <Form
          className="mb-3 viewData-file-form"
          id="viewDataFileForm"
          inline={true}
    ...
    if (allowDownload && proxyDownloadApiUrl && path) {
      return (
        <FormGroup className="col-4 mt-2">
          <a
            href={`${proxyDownloadApiUrl.prefix}${path}${proxyDownloadApiUrl.suffix}`}
          >
