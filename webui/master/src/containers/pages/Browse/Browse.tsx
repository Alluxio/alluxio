  ...
  private renderFileView(
    ...
    return (
      <React.Fragment>
        ...
        <hr />
        <h6>
          Detailed blocks information (block capacity is{' '}
          {browseData.blockSizeBytes} Bytes):
        </h6>
        <Table hover={true}>
          <thead>
            <tr>
      ...
      <React.Fragment>
        ...
        >
          ...
          <FormGroup className="mb-2 mr-sm-2">
            <Button
              tag={Link}
              to={`/browse?path=${encodeURIComponent(path || '/')}`}
              color="secondary"
            >
              Go
            </Button>
            ...
            {fileInfos &&
              fileInfos.map((fileInfo: IFileInfo) => (
                <tr key={fileInfo.absolutePath}>
                  <td>
                    <FontAwesomeIcon
                      icon={fileInfo.isDirectory ? faFolder : faFile}
                    />
                  </td>
                  <td>
