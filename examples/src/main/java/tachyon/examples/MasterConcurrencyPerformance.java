...
public final class MasterConcurrencyPerformance {
  ...
  static {
     ...
     * {@link OperationCallable} for this type below in this static block.
     */
  ...
  /**
   * Default root directory to put all files generated by this performance test, can be changed by
   * command line option.
   */
   ...
   */
  private static void ensureEmptyDir(TachyonURI dir) throws IOException, TachyonException {
    try {
      TachyonFile fd = sTfs.open(dir);
      sTfs.delete(fd, new DeleteOptions.Builder(ClientContext.getConf()).setRecursive(true)
          .build());
    } catch (IOException ioe) {
      throw ioe;
    } catch (TachyonException te) {
      // It's expected that the directory may not exist.
    }
    sTfs.mkdir(dir, new MkdirOptions.Builder(ClientContext.getConf()).setRecursive(true).build());
  }

  /**
   * Creates files by a bunch of clients concurrently with directory structure like
  ...
  private static void createEmptyFilesUnderRoot(int nFiles, int nClients)
      throws Exception {
    TachyonURI testRootDir = new TachyonURI(PathUtils.concatPath(sTestRootDir,
        "createEmptyFilesUnderRoot"));
    ensureEmptyDir(testRootDir);
    ...
    for (int client = 0; client < nClients; client ++) {
      ...
      for (int file = 0; file < files; file ++) {
        paths.add(new TachyonURI(PathUtils.concatPath(testRootDir, client, file)));
      }
    ...
    System.out.println(FormatUtils.formatTimeTakenMs(startTimeMs, "create " + nFiles + " files by "
        + nClients + " clients concurrently."));
  }
