...
public class MakeConsistencyCommand extends AbstractShellCommand {
  ...
  public void run(CommandLine cl) throws AlluxioException, IOException {
    ...
    if (inconsistentUris.isEmpty()) {
      ...
      System.out.println(root + " have: " + inconsistentUris.size() + "files inconsistent.");
      for (int i = 0; i < inconsistentUris.size(); i++) {
        System.out.println("Fixing path: " + inconsistentUris.get(i).getPath());
        DeleteOptions deleteOptions = DeleteOptions.defaults().setRemoveUFSFile(false);
        ...
        System.out.println(inconsistentUris.get(i).getPath() + "fixed");
        System.out.println();
      }
  ...
  private void loadFile(AlluxioURI filePath) throws AlluxioException, IOException {
    ...
    if (!status.isFolder()) {
      System.out.println("The " + filePath + " is new added in UFS! success loaded");
    }
  }
