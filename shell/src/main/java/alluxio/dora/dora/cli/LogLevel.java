...
public final class LogLevel {
  ...
  private static List<TargetInfo> getTargetInfos(String[] targets, AlluxioConfiguration conf)
    ...
    for (String target : targetSet) {
      if (target.isEmpty()) {
        ...
        int masterPort = NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.MASTER_WEB, conf);
        ...
        int jobMasterPort = NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.JOB_MASTER_WEB, conf);
  ...
  private static String inferRoleFromPort(int port, AlluxioConfiguration conf) {
    if (port == NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.MASTER_WEB, conf)) {
      return ROLE_MASTER;
    } else if (port == NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.WORKER_WEB, conf)) {
      return ROLE_WORKER;
    } else if (port == NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.JOB_MASTER_WEB, conf)) {
      return ROLE_JOB_MASTER;
    } else if (port == NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.JOB_WORKER_WEB, conf)) {
