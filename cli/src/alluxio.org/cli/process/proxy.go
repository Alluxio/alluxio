...
func (p *ProxyProcess) InitCommandTree(processCmd *cobra.Command) {
	proxyCmd := &cobra.Command{
		Use:   Proxy.Name,
		Short: "Manages the Alluxio proxy process",
	}
	processCmd.AddCommand(proxyCmd)

	startCmd := env.NewProcessStartCmd(p)
	proxyCmd.AddCommand(startCmd)

	proxyCmd.AddCommand(env.NewProcessStopCmd(p))
}
...
func (p *ProxyProcess) Start() error {
