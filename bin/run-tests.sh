...
if [[ "$1" == "Basic" ]]; then
  java -cp $bin/../conf/:$TACHYON_JAR tachyon.examples.BasicUserOperationTest $MASTER_ADDRESS:9998 /Basic_File_$2 $2
  exit 0
elif [[ "$1" == "BasicRawTable" ]]; then
  java -cp $bin/../conf/:$TACHYON_JAR tachyon.examples.BasicRawTableTest $MASTER_ADDRESS:9998 /Basic_Raw_Table_$2 $2
  exit 0
fi
