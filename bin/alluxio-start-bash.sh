...
do_mount() {
  ...
  case "$1" in
    Mount|SudoMount)
      local tier_alias=$(${BIN}/alluxio-bash getConf alluxio.worker.tieredstore.level0.alias)
      local tier_path
      get_ramdisk_array
      ...
      do
        ...
        if [[ $? -eq 0 ]]; then
          ...
          ${LAUNCHER} "${BIN}/alluxio-mount-bash.sh" "$1"
          MOUNT_FAILED=$?
        fi
      done
      ;;
...
stop() {
  ${BIN}/alluxio-stop-bash.sh $1
}

start_job_master() {
  if [[ "$1" == "-f" ]]; then
    ${LAUNCHER} "${BIN}/alluxio-bash" format
  fi

  echo "Starting job master @ $(hostname -f). Logging to ${ALLUXIO_LOGS_DIR}"
  (nohup ${BIN}/launch-process-bash job_master > ${ALLUXIO_LOGS_DIR}/job_master.out 2>&1) &
}

start_job_masters() {
  ${LAUNCHER} "${BIN}/alluxio-masters-bash.sh" "${BIN}/alluxio-start-bash.sh" "-a" "job_master"
}

start_job_worker() {
  echo "Starting job worker @ $(hostname -f). Logging to ${ALLUXIO_LOGS_DIR}"
  (nohup ${BIN}/launch-process-bash job_worker > ${ALLUXIO_LOGS_DIR}/job_worker.out 2>&1) &
...
start_job_workers() {
  ${LAUNCHER} "${BIN}/alluxio-workers-bash.sh" "${BIN}/alluxio-start-bash.sh" "-a" "job_worker"
}

start_master() {
  if [[ "$1" == "-f" ]]; then
    ${LAUNCHER} ${BIN}/alluxio-bash format
  elif [[ `${LAUNCHER} ${BIN}/alluxio-bash getConf ${ALLUXIO_MASTER_JAVA_OPTS} alluxio.master.journal.type` == "EMBEDDED" ]]; then
    JOURNAL_DIR=`${LAUNCHER} ${BIN}/alluxio-bash getConf ${ALLUXIO_MASTER_JAVA_OPTS} alluxio.master.journal.folder`
    if [ -f "${JOURNAL_DIR}" ]; then
      ...
      ${LAUNCHER} ${BIN}/alluxio-bash formatMaster
    fi
  fi

  echo "Starting master @ $(hostname -f). Logging to ${ALLUXIO_LOGS_DIR}"
  (nohup ${BIN}/launch-process-bash master > ${ALLUXIO_LOGS_DIR}/master.out 2>&1) &
}

start_masters() {
  ${LAUNCHER} "${BIN}/alluxio-masters-bash.sh" "${BIN}/alluxio-start-bash.sh" "-a" "master" $1
}

start_proxy() {
  echo "Starting proxy @ $(hostname -f). Logging to ${ALLUXIO_LOGS_DIR}"
  (nohup ${BIN}/launch-process-bash proxy > ${ALLUXIO_LOGS_DIR}/proxy.out 2>&1) &
}

start_proxies() {
  ${LAUNCHER} "${BIN}/alluxio-masters-bash.sh" "${BIN}/alluxio-start-bash.sh" "-a" "proxy"
  ${LAUNCHER} "${BIN}/alluxio-workers-bash.sh" "${BIN}/alluxio-start-bash.sh" "-a" "proxy"
}
...
start_worker() {
  ...
  if [[ ! -z "${cache}" ]] ; then
    ...
    if [[ -z $(ls -a "${cache}" ) ]]; then
      ...
      get_ramdisk_array # see alluxio-common-bash.sh
  ...
  (nohup ${BIN}/launch-process-bash worker > ${ALLUXIO_LOGS_DIR}/worker.out 2>&1 ) &
}
...
start_workers() {
  ...
  ${LAUNCHER} "${BIN}/alluxio-workers-bash.sh" "${BIN}/alluxio-start-bash.sh" "-a" "worker" $1 ${start_opts}
}
...
start_monitor() {
  ...
  local run=
  if [[ -z "${run}" ]]; then
    ${LAUNCHER} "${BIN}/alluxio-monitor-bash.sh" "${action}" "${nodes}"
  else
