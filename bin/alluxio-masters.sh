...
for master in $(echo ${HOSTLIST}); do
  ...
  if [[ ${ZOOKEEPER_ENABLED} == "true" || ${N} -eq 0 ]]; then
    nohup ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -tt ${master} ${LAUNCHER} \
      $"source \$HOME/.bash_profile; ${@// /\\ }" 2>&1 | while read line; do echo "[${master}] ${line}"; done >> ${ALLUXIO_TASK_LOG} &
  else
    nohup ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -tt ${master} ${LAUNCHER} \
      $"source \$HOME/.bash_profile; export ALLUXIO_MASTER_SECONDARY=true; ${@// /\\ }" 2>&1 | while read line; do echo "[${master}] ${line}"; done >> ${ALLUXIO_TASK_LOG} &
  fi
  N=$((N+1))
done
