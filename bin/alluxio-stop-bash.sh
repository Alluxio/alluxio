...
USAGE="Usage: alluxio-stop-bash.sh [-h] [component] [-c cache] [-s]
...
generate_kill_command() {
  ...
  KILL_COMMAND+=("${BIN}/alluxio-bash" "killAll")
...
generate_cluster_kill_command() {
  ...
  if [[ "${script_type}" = "master" ]]; then
    KILL_COMMAND+=("${BIN}/alluxio-masters-bash.sh" "${BIN}/alluxio-stop-bash.sh")
  elif [[ "${script_type}" = "worker" ]]; then
    KILL_COMMAND+=("${BIN}/alluxio-workers-bash.sh" "${BIN}/alluxio-stop-bash.sh")
  else
