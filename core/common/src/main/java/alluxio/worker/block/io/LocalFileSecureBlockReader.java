 ...
 */
@NotThreadSafe
public class LocalFileSecureBlockReader extends BlockReader {
  ...
  /**
   * Constructs a Block reader given the file descriptor of the block.
   *
   * @param fd file descriptor of the block
   */
  public LocalFileSecureBlockReader(FileDescriptor fd) throws IOException {
    mFileInputStream = mCloser.register(new FileInputStream(fd));
    mFileSize = mFileInputStream.getChannel().size();
    mLocalFileChannel = mCloser.register(mFileInputStream.getChannel());
  }
