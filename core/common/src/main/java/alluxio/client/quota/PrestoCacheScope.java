...
/**
 * This class defines "Scope" of a Presto cache quota. For Presto caching, scope is
 ...
 */
@ThreadSafe
public class PrestoCacheScope implements CacheScope {
  private static final String GLOBAL_ID = ".";
  public static final String SEPARATOR = ".";
  public static final PrestoCacheScope GLOBAL = new PrestoCacheScope(GLOBAL_ID, 1, Level.GLOBAL);
   ...
   */
  @Nullable
  public static PrestoCacheScope create(String id) {
    ...
    if (GLOBAL_ID.equals(id)) {
      ...
      return new PrestoCacheScope(id, id.length(), Level.SCHEMA);
    } else if (Level.TABLE.matches(id)) {
      return new PrestoCacheScope(id, id.length(), Level.TABLE);
    } else if (Level.PARTITION.matches(id)) {
      return new PrestoCacheScope(id, id.length(), Level.PARTITION);
    }
    return null;
  }

  private PrestoCacheScope(String id, int len, Level level) {
    mId = id;
    mLength = len;
    mLevel = level;
  }
  ...
  @Override
  @Nullable
  public PrestoCacheScope parent() {
    ...
    return new PrestoCacheScope(mId, r, mLevel.parent());
  }
  ...
  public boolean equals(Object o) {
    ...
    PrestoCacheScope cacheScope = (PrestoCacheScope) o;
