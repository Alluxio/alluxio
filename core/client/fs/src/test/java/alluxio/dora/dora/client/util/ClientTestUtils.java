...
package alluxio.dora.dora.client.util;
...
public final class ClientTestUtils {
  ...
  public static BlockWorkerInfo worker(long capacity, long used, String node, String rack) {
    ...
    List<TieredIdentity.LocalityTier> tiers = new ArrayList<>();
    if (node != null && !node.isEmpty()) {
      address.setHost(node);
      tiers.add(new TieredIdentity.LocalityTier(Constants.LOCALITY_NODE, node));
    }
    if (rack != null && !rack.isEmpty()) {
      tiers.add(new TieredIdentity.LocalityTier(Constants.LOCALITY_RACK, rack));
    }
