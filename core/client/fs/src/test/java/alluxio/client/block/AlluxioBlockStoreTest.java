...
public final class AlluxioBlockStoreTest {
  ...
  public void getInStreamLocal() throws Exception {
    ...
    assertEquals(local,
        mBlockStore.getInStream(BLOCK_ID, new InStreamOptions(new URIStatus(new FileInfo())))
            .getAddress());
  }
  ...
  public void getInStreamRemote() throws Exception {
    ...
    for (int i = 0; i < 40; i++) {
      results.add(mBlockStore.getInStream(BLOCK_ID,
          new InStreamOptions(new URIStatus(new FileInfo()))).getAddress());
    }
