...
package alluxio.dora.dora.client.block.policy;
...
import alluxio.dora.dora.util.TieredIdentityUtils;
 ...
 *
 * The definition of 'nearest worker' is based on {@link TieredIdentity}.
 * @see TieredIdentityUtils#nearest
 *
 * The calculation of which worker gets selected is done for each block write.
 *
 * The {@link PropertyKey.USER_BLOCK_AVOID_EVICTION_POLICY_RESERVED_BYTES}
...
public final class LocalFirstAvoidEvictionPolicy implements BlockLocationPolicy {
  ...
  public LocalFirstAvoidEvictionPolicy(AlluxioConfiguration conf) {
    this(conf.getBytes(PropertyKey.USER_BLOCK_AVOID_EVICTION_POLICY_RESERVED_BYTES),
        conf);
  }
   ...
   * {@link PropertyKey.USER_BLOCK_AVOID_EVICTION_POLICY_RESERVED_BYTES}
   * (alluxio.user.block.avoid.eviction.policy.reserved.size.bytes)
   *
