...
public class FileInStreamTest {
  ...
  public void testSeekWithCachingIncompleteBlocks() throws IOException {
    mTestStream = new FileInStream(mStatus,
      InStreamOptions.defaults().setReadType(ReadType.CACHE_PROMOTE).enableCacheIncompleteBlock());
    int seekAmount = (int) (BLOCK_LENGTH / 2);
    ...
    Assert
      .assertArrayEquals(BufferUtils.getIncreasingByteArray((int) BLOCK_LENGTH, (int) BLOCK_LENGTH),
        mCacheStreams.get(1).getWrittenData());
