...
import alluxio.dora.dora.security.GroupMappingServiceTestUtils;
...
import alluxio.dora.dora.security.group.GroupMappingService;
...
public final class PermissionCheckerTest {
  ...
  private static void createAndSetPermission(String path, CreateFileContext context)
    ...
    try (LockedInodePath inodePath =
        sTree.lockInodePath(
            new AlluxioURI(path), InodeTree.LockPattern.WRITE_EDGE, NoopJournalContext.INSTANCE)
    ) {
  ...
  public void invalidPath() throws Exception {
    ...
    try (LockedInodePath inodePath = sTree
        .lockInodePath(new AlluxioURI(""), InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
  ...
  public void getPermission() throws Exception {
    try (LockedInodePath path =
             sTree.lockInodePath(
                 new AlluxioURI(TEST_WEIRD_FILE_URI), InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)
    ) {
      // user is admin
  ...
  private void checkPermission(TestUser user, Mode.Bits action, String path)
    ...
    try (LockedInodePath inodePath = sTree
        .lockInodePath(new AlluxioURI(path), InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
