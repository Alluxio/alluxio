...
public final class InodeTreeTest {
  ...
  public void createFileUnderPinnedDirectory() throws Exception {
    ...
    try (
        LockedInodePath inodePath = mTree.lockFullInodePath(
            NESTED_URI, InodeTree.LockPattern.WRITE_INODE, NoopJournalContext.INSTANCE)
    ) {
  ...
  public void getInodeByInvalidId() throws Exception {
    ...
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(1, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
      // inode exists
    }
  }
  ...
  public void getPath() throws Exception {
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(0, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
    ...
    createPath(mTree, TEST_URI, sDirectoryContext);
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(TEST_URI, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
    ...
    try (LockedInodePath inodePath = mTree.lockFullInodePath(
        NESTED_URI, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)
    ) {
  ...
  public void checkChildren(List<AlluxioURI> files) throws Exception {
    ...
    for (AlluxioURI nxt : files) {
      try (LockedInodePath inodePath = mTree.lockFullInodePath(
          nxt, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
  ...
  public void getChildrenPrefix() throws Exception {
    ...
    try (LockedInodePath inodePath = mTree.lockFullInodePath(
        file, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
      long parentId = inodePath.getInode().getParentId();
      Inode[] childInodes;
  ...
  public void getInodeChildrenRecursive() throws Exception {
    ...
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(0, InodeTree.LockPattern.WRITE_INODE, NoopJournalContext.INSTANCE)) {
  ...
  public void addInodeFromJournal() throws Exception {
    ...
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(
                 new AlluxioURI("/"), InodeTree.LockPattern.WRITE_INODE, NoopJournalContext.INSTANCE)
    ) {
  ...
  public void getInodePathById() throws Exception {
    ...
    for (Inode inode : created) {
      long id = inode.getId();
      try (LockedInodePath inodePath
               = mTree.lockFullInodePath(id, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
  ...
  public void getInodePathByPath() throws Exception {
    try (LockedInodePath rootPath =
        mTree.lockFullInodePath(
            new AlluxioURI("/"), InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)
    ) {
    ...
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(uri, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
  ...
  private List<Inode> createPath(InodeTree root, AlluxioURI path, CreatePathContext<?, ?> context)
    ...
    try (LockedInodePath inodePath =
             root.lockInodePath(path, InodeTree.LockPattern.WRITE_EDGE, NoopJournalContext.INSTANCE)) {
  ...
  private MutableInode<?> getInodeByPath(AlluxioURI path) throws Exception {
    try (LockedInodePath inodePath =
             mTree.lockFullInodePath(path, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)) {
  ...
  private static void deleteInodeByPath(InodeTree root, AlluxioURI path) throws Exception {
    try (LockedInodePath inodePath
             = root.lockFullInodePath(path, InodeTree.LockPattern.WRITE_EDGE, NoopJournalContext.INSTANCE)) {
