...
import alluxio.dora.proto.journal.File.UpdateInodeEntry.Builder;
...
public class InodeTreePersistentState implements Journaled {
    ...
    if (inode.isFile()) {
      ...
      if (pinned) {
        ...
        if (file.getReplicationMax() == 0) {
          file.setReplicationMax(Constants.REPLICATION_MAX_INFINITY);
        }
      ...
      if (file.getReplicationMax() != Constants.REPLICATION_MAX_INFINITY) {
  ...
  private void applyUpdateInodeFile(UpdateInodeFileEntry entry) {
    ...
    if (entry.hasReplicationMax()) {
      if (entry.getReplicationMax() == Constants.REPLICATION_MAX_INFINITY) {
