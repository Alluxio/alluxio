...
import alluxio.dora.dora.concurrent.LockMode;
...
import alluxio.dora.dora.master.file.InodeSyncStream;
...
import alluxio.dora.dora.resource.AlluxioResourceLeakDetectorFactory;
...
public class LockedInodePath implements Closeable {
   ...
   * {@link MetadataSyncMergeJournalContext} only
   ...
   * if {@link FileSystemMergeJournalContext} is used because
  ...
  public LockedInodePath(AlluxioURI uri, ReadOnlyInodeStore inodeStore,
      InodeLockManager inodeLockManager, InodeDirectory root, InodeTree.LockPattern lockPattern,
  ...
  private LockedInodePath(AlluxioURI uri, LockedInodePath path, String[] pathComponents,
                          InodeTree.LockPattern lockPattern, boolean tryLock) {
  ...
  /**
   * @return the {@link InodeTree.LockPattern} of this path
   */
  ...
  public void addNextInode(Inode inode) {
    Preconditions.checkState(mLockPattern == InodeTree.LockPattern.WRITE_EDGE);
    Preconditions.checkState(!fullPathExists());
  ...
  public void downgradeToRead() {
    ...
    mLockPattern = InodeTree.LockPattern.READ;
  }
   ...
   */
  public LockedInodePath lockDescendant(AlluxioURI descendantUri, InodeTree.LockPattern lockPattern)
      throws InvalidPathException {
   ...
   * @return the new locked path
   */
  public LockedInodePath lockChild(Inode child, InodeTree.LockPattern lockPattern)
      throws InvalidPathException {
    return lockChild(child, lockPattern, addComponent(mPathComponents, child.getName()));
  }

  /**
   * Efficient version of {@link #lockChild(Inode, InodeTree.LockPattern)} for when the child path
   ...
   * @return the new locked path
   */
  public LockedInodePath lockChildByName(String childName, InodeTree.LockPattern lockPattern,
      String[] childComponentsHint) throws InvalidPathException {
  ...
  public LockedInodePath lockFinalEdgeWrite() throws InvalidPathException {
    ...
    LockedInodePath newPath =
        new LockedInodePath(mUri, this, mPathComponents, InodeTree.LockPattern.WRITE_EDGE, mUseTryLock);
    newPath.traverse();
    return newPath;
  }
   ...
   * locked according to {@link InodeTree.LockPattern}.
   *
   ...
   * {@link InodeSyncStream}. Where traverse() is called right before the
  ...
  public void traverse() throws InvalidPathException {
    ...
    while (!fullPathExists()) {
      ...
      if (mLockList.endsInInode()) { // Lock an edge next.
        if (mLockPattern == InodeTree.LockPattern.WRITE_EDGE && isFinalComponent) {
        ...
        if (!nextInodeOpt.isPresent() && mLockPattern == InodeTree.LockPattern.WRITE_EDGE
            && !isFinalComponent) {
        ...
        if (!nextInodeOpt.isPresent()) {
          if (mLockPattern == InodeTree.LockPattern.READ) {
  ...
  private void bootstrapTraversal() {
    ...
    if (mPathComponents.length == 1) {
      if (mLockPattern == InodeTree.LockPattern.WRITE_EDGE) {
        edgeLock = LockMode.WRITE;
        inodeLock = LockMode.WRITE;
      } else if (mLockPattern == InodeTree.LockPattern.WRITE_INODE) {
        inodeLock = LockMode.WRITE;
      }
    }
