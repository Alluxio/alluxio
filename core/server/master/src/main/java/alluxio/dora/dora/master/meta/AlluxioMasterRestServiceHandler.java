...
import static alluxio.dora.dora.metrics.MetricInfo.UFS_OP_PREFIX;
import static alluxio.dora.dora.metrics.MetricInfo.UFS_OP_SAVED_PREFIX;
...
import alluxio.dora.dora.web.MasterWebServer;
import alluxio.dora.dora.wire.MasterInfo;
...
import alluxio.dora.dora.util.webui.NodeInfo;
...
import alluxio.dora.dora.wire.AlluxioMasterInfo;
...
import alluxio.dora.dora.wire.MasterWebUIBrowse;
import alluxio.dora.dora.wire.MasterWebUIConfiguration;
import alluxio.dora.dora.wire.MasterWebUIData;
import alluxio.dora.dora.wire.MasterWebUIInit;
import alluxio.dora.dora.wire.MasterWebUILogs;
import alluxio.dora.dora.wire.MasterWebUIMasters;
import alluxio.dora.dora.wire.MasterWebUIMetrics;
import alluxio.dora.dora.wire.MasterWebUIMountTable;
import alluxio.dora.dora.wire.MasterWebUIOverview;
import alluxio.dora.dora.wire.MasterWebUIWorkers;
...
public final class AlluxioMasterRestServiceHandler {
  ...
  @ApiOperation(value = "Get general Alluxio Master service information",
      response = AlluxioMasterInfo.class)
  ...
  public Response getWebUIMasters() {
    return RestUtils.call(() -> {
      ...
      alluxio.dora.dora.wire.MasterInfo[] failedMasterInfos = mMetaMaster.getLostMasterInfos();
      response.setFailedMasterInfos(failedMasterInfos);

      alluxio.dora.dora.wire.MasterInfo[] normalMasterInfos = mMetaMaster.getMasterInfos();
      response.setNormalMasterInfos(normalMasterInfos);

      InetSocketAddress leaderMasterAddress = mMasterProcess.getRpcAddress();
      alluxio.dora.dora.wire.MasterInfo leaderMasterInfo = new MasterInfo(MASTER_ID_NULL,
    ...
    return RestUtils.call(() -> {
      ...
      for (Map.Entry<String, Counter> entry : counters.entrySet()) {
        ...
        if (metricName.contains(MetricKey.CLUSTER_BYTES_READ_UFS.getName())) {
          String ufs = alluxio.dora.dora.metrics.Metric.getTagUfsValueFromFullName(metricName);
          ...
          if (ufs != null && isMounted(ufs)) {
            ...
            String alluxioOperation = alluxio.dora.dora.metrics.Metric.getBaseName(metricName)
      ...
      for (Map.Entry<String, Gauge> entry : gauges.entrySet()) {
        ...
        if (metricName.contains(UFS_OP_PREFIX)) {
          ...
          if (ufs != null && isMounted(ufs)) {
            ...
            perUfsMap.put(alluxio.dora.dora.metrics.Metric.getBaseName(metricName)
