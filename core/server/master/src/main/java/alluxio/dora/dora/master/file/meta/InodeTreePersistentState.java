...
import alluxio.dora.dora.util.BucketCounter;
...
public class InodeTreePersistentState implements Journaled {
  ...
  public InodeTreePersistentState(InodeStore inodeStore, InodeLockManager lockManager,
    ...
    mBucketCounter = new BucketCounter(
        Configuration.getList(PropertyKey.MASTER_METRICS_FILE_SIZE_DISTRIBUTION_BUCKETS)
            .stream().map(FormatUtils::parseSpaceSize).collect(Collectors.toList()));
    mRetryCacheEnabled =
        Configuration.getBoolean(PropertyKey.MASTER_FILE_SYSTEM_OPERATION_RETRY_CACHE_ENABLED);
    mOpIdCache = CacheBuilder.newBuilder()
        .maximumSize(Configuration.getInt(PropertyKey.MASTER_FILE_SYSTEM_OPERATION_RETRY_CACHE_SIZE))
        .build();
  }
