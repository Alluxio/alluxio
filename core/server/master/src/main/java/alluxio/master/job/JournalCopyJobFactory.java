...
public class JournalCopyJobFactory implements JobFactory {
  ...
  public Job<?> create() {
    ...
    FileIterable fileIterator =
        new FileIterable(mFsMaster, mJobEntry.getSrc(), user, mJobEntry.getPartialListing(),
            LoadJob.QUALIFIED_FILE_FILTER);
    CopyJob job = new CopyJob(mJobEntry.getSrc(), mJobEntry.getDst(), user, mJobEntry.getJobId(),
