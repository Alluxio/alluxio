      ...
      onRegisterApi: function (gridApi) {
        ...
        $scope.gridApi.core.on.sortChanged($scope, function (grid, sortColumns) {
          ...
          $.each(sortColumns, function (index, value) {
            paginationOptions.sorters.push({"field": value.field, "direction": value.sort.direction});
          });
          generateData($scope);
        });
        ...
        $scope.gridApi.core.on.filterChanged($scope, function () {
          ...
          $.each(grid.columns, function (index, value) {
            if (value.filters[0].term != undefined && value.filters[0].term != "") {
              paginationOptions.filters.push({"field": value.field, "term": value.filters[0].term});
      ...
      columnDefs: [
        {
          ...
          cellTemplate: '<nobr>' +
          '<i class="icon-file"></i>' +
          '<a href="javascript:void(0)" ng-click=grid.appScope.openFile(row)>{{row.entity.absolutePath}}</a>' +
          '</nobr>',
