...
public final class MasterWebServer extends WebServer {
  ...
  public MasterWebServer(String serviceName, InetSocketAddress address,
    ...
    mWebAppContext.addServlet(new ServletHolder(new WebInterfaceBrowseAjaxServlet(master)), "/browse/jumpPath.ajax");

    mWebAppContext.addServlet(new ServletHolder(new WebInterfaceMemoryServlet(master)), "/memory");
    mWebAppContext.addServlet(new ServletHolder(new WebInterfaceMemoryAjaxServlet(master)), "/memory/getFiles.ajax");
