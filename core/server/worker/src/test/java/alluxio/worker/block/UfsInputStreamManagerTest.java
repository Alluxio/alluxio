...
public class UfsInputStreamManagerTest {
  ...
  public ConfigurationRule mConfigurationRule =
      new ConfigurationRule(new HashMap<PropertyKey, String>() {
        {
          put(PropertyKey.WORKER_UFS_INSTREAM_CACHE_EXPIRE_MS, "200000");
        }
      });
  ...
  public void before() throws Exception {
    ...
    for(int i=0;i<3;i++) {
      ...
      Mockito.when(mUfs.open(Mockito.eq(FILE_NAME), Mockito.any(OpenOptions.class))).thenReturn(instream);
