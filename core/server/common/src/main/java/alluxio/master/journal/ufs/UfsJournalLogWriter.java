...
final class UfsJournalLogWriter implements JournalWriter {
  ...
  private void createNewLogFile(long sequenceNumber) throws IOException {
    URI newLog = UfsJournalFile
        .encodeLogFileLocation(mJournal, sequenceNumber, UfsJournal.UNKNOWN_SEQUENCE_NUMBER);
    UfsJournalFile currentLog = UfsJournalFile.createLogFile(newLog, sequenceNumber,
