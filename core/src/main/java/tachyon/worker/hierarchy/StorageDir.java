...
public class StorageDir {
  ...
  private final Map<Long, Long> mLatestBlockAccessTimeMs = new HashMap<Long, Long>();// TODO
  ...
  private final Map<Long, Long> mUserAllocatedSpace = new HashMap<Long, Long>();// no use currently
  ...
  public void checkStatus(List<Long> removedUsers) {
    for (long userId : removedUsers) {
      // TODO delete user related directory and reallocate space. delete user outside.
  ...
  public BlockHandler getBlockHandler(long blockId) throws IOException {
    ...
    try {
      ...
      throw new IOException(e.getMessage()); // TODO better way to handle Exception
    }
  }
  ...
  public boolean moveBlock(long blockId, StorageDir dst) throws IOException {
    ...
    /*
     * String srcFile = getBlockFilePath(blockId);
     * String dstFile = dst.getBlockFilePath(blockId);
     * long blockSize = getBlockSize(blockId);
     ...
     * CommonUtils.changeLocalFileToFullPermission(srcFile);
     * // use the sticky bit, only the client and the worker can write to the block
     * CommonUtils.setLocalFileStickyBit(srcFile);
     * boolean result = srcUfs.rename(srcFile, dstFile); //failed to rename
     * if(result) {
     * deleteBlockId(blockId);
     * dst.addBlockId(blockId, blockSize);
     * }
     * return result;
     * } else {
     */
