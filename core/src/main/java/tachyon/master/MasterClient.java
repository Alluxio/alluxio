...
public final class MasterClient implements Closeable {
  ...
  /**
   * Clean the connect. E.g. if the client has not connected with the master for a while, the
  ...
  public synchronized void connect() throws IOException {
    ...
    do {
      ...
      LOG.info("Tachyon client (version " + Version.VERSION + ") is trying to connect to master @ "
          + mMasterAddress);
      ...
      try {
        ...
        lastException = e;
        LOG.error("Failed to connect (" + retry.getRetryCount() + ") to master @ "
