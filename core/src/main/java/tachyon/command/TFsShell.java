...
public class TFsShell implements Closeable {
  ...
  public int cat(String argv[]) throws IOException {
    ...
    if (tFile == null) {
      System.out.println(path.getPath() + " does not exist.");
      return -1;
    }
    if (tFile.isFile()) {
      ...
      System.out.println(path.getPath() + " is not a file.");
      return -1;
    }
  }
  ...
  public int copyToLocal(String argv[]) throws IOException {
    ...
    if (tFile == null) {
      throw new IOException(srcPath.getPath());
    }
    ...
    try {
      ...
      System.out.println("Copied " + srcPath.getPath() + " to " + dstPath);
      return 0;
    } finally {
      closer.close();
    }
  ...
  public int fileinfo(String argv[]) throws IOException {
    ...
    System.out.println(path.getPath() + " with file id " + fileId + " has the following blocks: ");
  ...
  public int location(String argv[]) throws IOException {
    ...
    System.out.println(path.getPath() + " with file id " + fileId + " is on nodes: ");
  ...
  public int mkdir(String argv[]) throws IOException {
    ...
    if (tachyonClient.mkdir(path)) {
      System.out.println("Successfully created directory " + path.getPath());
      return 0;
    } else {
      return -1;
    }
  ...
  public int pin(String argv[]) throws IOException {
    ...
    try {
      tachyonClient.pinFile(fileId);
      System.out.println("File '" + path.getPath() + "' was successfully pinned.");
      return 0;
    } catch (Exception e) {
      e.printStackTrace();
      System.out.println("File '" + path.getPath() + "' could not be pinned.");
      return -1;
    }
  }
  ...
  public int rename(String argv[]) throws IOException {
    ...
    if (tachyonClient.rename(srcPath, dstPath)) {
      System.out.println("Renamed " + srcPath.getPath() + " to " + dstPath.getPath());
      return 0;
    } else {
      return -1;
    }
  ...
  public int report(String argv[]) throws IOException {
    ...
    System.out.println(
        path.getPath() + " with file id " + fileId + " has reported been report lost.");
    return 0;
  }
  ...
  public int unpin(String argv[]) throws IOException {
    ...
    try {
      ...
      System.out.println("File '" + path.getPath() + "' was successfully unpinned.");
      return 0;
    } catch (Exception e) {
      e.printStackTrace();
      System.out.println("File '" + path.getPath() + "' could not be unpinned.");
      return -1;
    }
  }
