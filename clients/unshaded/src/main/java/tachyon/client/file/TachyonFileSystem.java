...
public class TachyonFileSystem extends AbstractTachyonFileSystem {
  ...
  /**
   * Create is not supported in StreamingTachyonFileSystem. Files should only be written once. Use
   * getOutStream instead to create files.
   */
  @Override
  ...
  /**
   * Convenience method for {@link #createEmptyFile(TachyonURI, ClientOptions)} with default client
   * options.
   *
  ...
  /**
   * @param path the Tachyon path of the file to load
   ...
   */
  public long loadFileInfoFromUfs(TachyonURI path, boolean recursive) throws IOException,
      FileDoesNotExistException {
    FileSystemMasterClient masterClient = mContext.acquireMasterClient();
    try {
      return masterClient.loadFileInfoFromUfs(path.getPath(), recursive);
    } finally {
  ...
  /**
   * Sets the pin status of a file. The file's pin status will be true regardless of whether or not
   * it was pinned previously. Pinning a file prevents any of its blocks from being evicted, but
   * does not load the blocks into memory. The blocks must be loaded through another means, for
   * example the load command in the shell. Calling this method is equivalent to calling
   * setPin(file, true).
   *
  ...
  /**
   * Unsets the pin status of a file. The file's pin status will be false regardless of whether or
   * not it was pinned previously. Unpinning a file makes it eligible for eviction, but the file
   * will not be evicted from Tachyon space until the eviction policy deems it necessary. Calling
   * this method is equivalent to calling setPin(file, false).
   *
