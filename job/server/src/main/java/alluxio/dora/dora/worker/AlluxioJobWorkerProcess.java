...
import alluxio.dora.dora.web.JobWorkerWebServer;
...
public final class AlluxioJobWorkerProcess implements JobWorkerProcess {
  ...
  AlluxioJobWorkerProcess() {
    try {
      ...
      // Setup web server
      mWebServer = new JobWorkerWebServer(NetworkAddressUtils.ServiceType.JOB_WORKER_WEB.getServiceName(),
          NetworkAddressUtils.getBindAddress(NetworkAddressUtils.ServiceType.JOB_WORKER_WEB,
              Configuration.global()),
          this);

      // Random port binding.
      InetSocketAddress configuredBindAddress =
              NetworkAddressUtils.getBindAddress(NetworkAddressUtils.ServiceType.JOB_WORKER_RPC,
                  Configuration.global());
      int rpcPort;
      ...
      mRpcBindAddress = NetworkAddressUtils.getBindAddress(NetworkAddressUtils.ServiceType.JOB_WORKER_RPC,
          Configuration.global());
      mRpcConnectAddress = NetworkAddressUtils.getConnectAddress(NetworkAddressUtils.ServiceType.JOB_WORKER_RPC,
          Configuration.global());
    } catch (Exception e) {
  ...
  public void start() throws Exception {
    ...
    LOG.info("Alluxio job worker version {} started. "
        ...
        NetworkAddressUtils.getConnectAddress(NetworkAddressUtils.ServiceType.JOB_WORKER_RPC,
            Configuration.global()),
        NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.JOB_WORKER_RPC, Configuration.global()),
        NetworkAddressUtils.getPort(NetworkAddressUtils.ServiceType.JOB_WORKER_WEB, Configuration.global()));
  ...
  public WorkerNetAddress getAddress() {
    return new WorkerNetAddress()
        .setHost(NetworkAddressUtils.getConnectHost(NetworkAddressUtils.ServiceType.JOB_WORKER_RPC,
            Configuration.global()))
