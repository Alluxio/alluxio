...
import alluxio.dora.dora.job.plan.batch.BatchedJobDefinition;
import alluxio.dora.dora.job.plan.load.LoadDefinition;
import alluxio.dora.dora.job.plan.load.LoadConfig;
import alluxio.dora.dora.job.plan.persist.PersistConfig;
...
public class BatchedJobDefinitionTest {
  ...
  public void batchLoad() throws Exception {
    ...
    for (Pair<WorkerInfo, BatchedJobDefinition.BatchedJobTask> assignment : assignments) {
      ArrayList<LoadDefinition.LoadTask> second = (ArrayList<LoadDefinition.LoadTask>) assignment.getSecond().getJobTaskArgs();
