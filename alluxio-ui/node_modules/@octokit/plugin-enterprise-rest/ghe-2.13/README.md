# @octokit/plugin-enterprise-rest/ghe-2.13
...
octokit.enterpriseAdmin.suspendUser(username)
...
octokit.enterpriseAdmin.unsuspendUser(username)
...
octokit.git.createCommit(author, committer, message, owner, parents, repo, tree)
...
octokit.projects.update(body, name, page, per_page, project_id, state)
...
octokit.repos.createCommitComment(body, line, owner, path, position, repo, sha)
...
octokit.repos.createFile(author, branch, committer, content, message, owner, path, repo)
...
octokit.repos.listCommentsForCommit(owner, page, per_page, ref, repo)
...
octokit.repos.update(allow_merge_commit, allow_rebase_merge, allow_squash_merge, archived, default_branch, description, has_issues, has_projects, has_wiki, homepage, name, owner, private, repo)
...
octokit.repos.updateProtectedBranchPullRequestReviewEnforcement(branch, dismiss_stale_reviews, dismissal_restrictions, owner, repo, require_code_owner_reviews)
