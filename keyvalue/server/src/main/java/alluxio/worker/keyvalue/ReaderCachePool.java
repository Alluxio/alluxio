...
public class ReaderCachePool {
  ...
  private Map<Long, FutureTask<CacheEntry>> mReaderMap = new ConcurrentHashMap<>();
  ...
  ByteBufferKeyValuePartitionReader getReader(final Long blockId)
      throws InvalidWorkerStateException, BlockDoesNotExistException, IOException {
    while (true) {
      FutureTask<CacheEntry> cacheEntry = mReaderMap.get(blockId);
      if (cacheEntry == null) {
        ...
        FutureTask<CacheEntry> cacheEntryFutureTask = new FutureTask<>(eval);
        cacheEntry = putIfAbsent(blockId, cacheEntryFutureTask);
        if (cacheEntry == null) {
          cacheEntry = cacheEntryFutureTask;
          cacheEntryFutureTask.run();
        }
      }
      try {
        return cacheEntry.get().getByteBufferKeyValuePartitionReader();
      } catch (InterruptedException e) {
        mReaderMap.remove(blockId, cacheEntry);
   ...
   * @throws BlockDoesNotExistException if the worker is not serving this block
   */
  private synchronized FutureTask<CacheEntry> putIfAbsent(final Long blockId,
      final FutureTask<CacheEntry> cacheEntry) throws IOException, BlockDoesNotExistException {
