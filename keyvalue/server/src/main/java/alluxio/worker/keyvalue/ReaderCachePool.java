 ...
 *  pool has a fixed size and uses a LRU Policy to evict when the pool is full.
 *  To avoid some entries resident in the pool for a long time, use a Evict Thread to evict
 *  the oldest entry once in a while.
 */
public class ReaderCachePool {
  ...
  public ReaderCachePool(int size, BlockWorker blockWorker) {
    ...
    mReaderMap = new ConcurrentHashMap<>();
  ...
  ByteBufferKeyValuePartitionReader getReader(final Long blockId)
    ...
    while (true) {
      ...
      try {
        ...
        LOG.error("cannot create the cacheEntry", e);
      }
    }
  }
  ...
  private void release(long blockId) throws IOException, BlockDoesNotExistException {
    ...
    try {
      oldValue = mReaderMap.get(blockId).get();
    } catch (InterruptedException e) {
      mReaderMap.remove(blockId);
    } catch (ExecutionException e) {
      LOG.error("create cacheEntry failed", e);
    }
    oldValue.getByteBufferKeyValuePartitionReader().close();
    oldValue.getBlockReader().close();
    mBlockWorker.unlockBlock(oldValue.getLockId());
  }
  ...
  /**
   * This thread is always running to evict the eldest entry to avoid some entry stay in the pool
   * for too long.
   */
  ...
  /**
   * Reader pool's entry, the lock is also in the entry. If the reader is in the pool,doesn't need
   * to lock the block to get readers.
   */
