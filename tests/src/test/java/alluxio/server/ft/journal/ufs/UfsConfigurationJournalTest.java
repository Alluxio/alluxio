...
public class UfsConfigurationJournalTest {
  ...
  public void testOptionsPersisted() throws Exception {
    ...
    try (CloseableResource<UnderFileSystem> resource =
      ...
      UnderFileSystem ufs = Whitebox.getInternalState(resource.get(), "mUnderFileSystem");
      UnderFileSystemConfiguration ufsConf = Whitebox.getInternalState(ufs, "mUfsConf");
      assertEquals(ufsConf.getUserSpecifiedConf().size(), options.size());
      for (Map.Entry<String, String> entry : options.entrySet()) {
        assertEquals(entry.getValue(), ufsConf.getUserSpecifiedConf().get(entry.getKey()));
      }
