...
// TODO(jiacheng): finish this set of UT
public class FileInStreamDecommissionIntegrationTest {
  ...
  public final void setUp() throws Exception {
    ...
    AlluxioURI path0 = new AlluxioURI(mTestPath + "/file_BOTH");
    FileSystemTestUtils.createByteFile(mFileSystem, path0, mWriteBoth, LENGTH);

    AlluxioURI path1 = new AlluxioURI(mTestPath + "/file_CACHE");
    FileSystemTestUtils.createByteFile(mFileSystem, path1, mWriteAlluxio, LENGTH);
    System.out.println("Existing files are:");
    System.out.println(mFileSystem.listStatus(new AlluxioURI(mTestPath)));
  ...
  public void readUfsFromUndecommissionedWorker() throws Exception {
    ...
    List<WorkerInfo> availableWorkers = context.acquireBlockMasterClientResource().get().getWorkerInfoList();
    assertEquals(2, availableWorkers.size());

    URIStatus status = context.acquireMasterClientResource().get().getStatus(uri, GetStatusPOptions.getDefaultInstance());
    ...
    DecommissionWorkerPOptions decomOptions = DecommissionWorkerPOptions.newBuilder()
            .setWorkerHostname(workerToDecommission.getHost()).setWorkerWebPort(workerToDecommission.getWebPort())
    ...
    // The 2nd block is read from the other worker, so there should be cache on the way
    URIStatus statusAfterRead = context.acquireMasterClientResource().get().getStatus(uri, GetStatusPOptions.getDefaultInstance());
    assertEquals(2, statusAfterRead.getFileBlockInfos().size());
    List<BlockLocation> block0Locs = statusAfterRead.getFileBlockInfos().get(0).getBlockInfo().getLocations();
    ...
    List<BlockLocation> block1Locs = statusAfterRead.getFileBlockInfos().get(1).getBlockInfo().getLocations();
    assertEquals(1, block0Locs.size());
    // The block is not on the decommissioned worker, meaning it is cached on the other worker
  ...
  public void decommissionWhileReading() throws Exception {
    ...
    mThreadPool.submit(() -> {
      try {
        ...
        List<WorkerInfo> updatedWorkers = context.acquireBlockMasterClientResource().get().getWorkerInfoList();
  ...
  public void halfStreamFromAnotherWorker() throws Exception {
    ...
    FileSystemContext context = FileSystemContext
            .create(new TestUserState("test", Configuration.global()).getSubject(),
                    Configuration.global());
    List<WorkerInfo> clusterWorkers = context.acquireBlockMasterClientResource().get().getWorkerInfoList();
    ...
    WorkerNetAddress cachedToWorker = block1.getBlockInfo().getLocations().get(0).getWorkerAddress();
