...
public class ForceMasterSyncAfterReadOutOfSyncTest
  ...
  public class TestFileInStream extends AlluxioFileInStream {

    protected TestFileInStream(URIStatus status,
  ...
  @Test
  public void forceMasterSync() throws Exception {
    mCluster = MultiProcessCluster.newBuilder(PortCoordination.EMBEDDED_JOURNAL_FAILOVER)
        .setClusterName("ForceMasterSyncAfterReadOutOfSyncTest_forceMasterSync")
    ...
    AlluxioURI testFile = new AlluxioURI("/test.txt");
    FileOutStream outStream = fs.createFile(testFile);
    String fileContent = "testing";
    outStream.write(fileContent.getBytes());
    outStream.close();

    URIStatus uriStatus = fs.getStatus(testFile);

    OpenFilePOptions readOptions =
        OpenFilePOptions.newBuilder().setReadType(ReadPType.CACHE_PROMOTE).build();

    TestFileInStream testFileInStream = new TestFileInStream(uriStatus,
        new InStreamOptions(uriStatus, readOptions, mConf),
        mCluster.getFilesystemContext());
    try {
      testFileInStream.read();
    } catch (Exception e) {
  ...
  public void restartStress() throws Throwable {
    ...
    FileSystem fs = mCluster.getFileSystemClient();
    List<ForceMasterSyncAfterReadOutOfSyncTest.OperationThread> threads = new ArrayList<>();
    try {
      for (int i = 0; i < 10; i++) {
        ForceMasterSyncAfterReadOutOfSyncTest.OperationThread t =
            new ForceMasterSyncAfterReadOutOfSyncTest
