...
package alluxio.dora.dora.client.fs;
...
import alluxio.dora.dora.exception.status.DeadlineExceededException;
import alluxio.dora.dora.exception.status.InternalException;
import alluxio.dora.dora.exception.status.NotFoundException;
import alluxio.dora.dora.grpc.GrpcExceptionUtils;
import alluxio.dora.dora.stress.cli.RpcBenchPreparationUtils;
import alluxio.dora.dora.stress.rpc.TierAlias;
import alluxio.dora.dora.util.ThreadFactoryUtils;
...
import alluxio.dora.dora.worker.block.BlockLockManager;
import alluxio.dora.dora.worker.block.BlockMasterClient;
import alluxio.dora.dora.worker.block.BlockMasterClientPool;
import alluxio.dora.dora.worker.block.BlockMasterSync;
import alluxio.dora.dora.worker.block.BlockMetadataManager;
import alluxio.dora.dora.worker.block.BlockStoreLocation;
import alluxio.dora.dora.worker.block.CreateBlockOptions;
import alluxio.dora.dora.worker.block.DefaultBlockWorker;
import alluxio.dora.dora.worker.block.MonoBlockStore;
import alluxio.dora.dora.worker.block.RegisterStreamer;
import alluxio.dora.dora.worker.block.TieredBlockReaderFactory;
import alluxio.dora.dora.worker.block.TieredBlockStore;
import alluxio.dora.dora.worker.block.TieredBlockWriterFactory;
import alluxio.dora.dora.worker.block.TieredTempBlockMetaFactory;
...
public class BlockWorkerRegisterStreamIntegrationTest {
  ...
  public void before() throws Exception {
    ...
    Configuration.set(PropertyKey.WORKER_REGISTER_STREAM_BATCH_SIZE, RegisterStreamTestUtils.BATCH_SIZE);
    ...
    // Prepare test data
    mTierAliases = RegisterStreamTestUtils.getTierAliases(RegisterStreamTestUtils.parseTierConfig(RegisterStreamTestUtils.TIER_CONFIG));
    mCapacityMap = Maps.toMap(mTierAliases, (tier) -> RpcBenchPreparationUtils.CAPACITY);
    mUsedMap = Maps.toMap(mTierAliases, (tier) -> 0L);
    mBlockMap = RpcBenchPreparationUtils.generateBlockIdOnTiers(RegisterStreamTestUtils.parseTierConfig(RegisterStreamTestUtils.TIER_CONFIG));
  ...
  public void requestsForEmptyWorker() throws Exception {
    ...
    assertEquals(RegisterStreamTestUtils.MEM_USAGE_EMPTY, request.getUsedBytesOnTiersMap());
    assertEquals(RegisterStreamTestUtils.MEM_CAPACITY, request.getTotalBytesOnTiersMap());
  ...
  public void requestsForWorker() throws Exception {
    ...
    // Verify the size and content of the requests
    int expectedBatchCount = (int) Math.ceil((RegisterStreamTestUtils.TIER_BLOCK_TOTAL) / (double) RegisterStreamTestUtils.BATCH_SIZE);
    ...
    for (int i = 0; i < expectedBatchCount; i++) {
      ...
      if (i != expectedBatchCount - 1) {
        Assert.assertEquals(RegisterStreamTestUtils.BATCH_SIZE, totalSize);
      }

      // The 1st request contains metadata but the following do not
      if (i == 0) {
        assertEquals(RegisterStreamTestUtils.USAGE_MAP, request.getUsedBytesOnTiersMap());
        assertEquals(RegisterStreamTestUtils.CAPACITY_MAP, request.getTotalBytesOnTiersMap());
        Map<String, StorageList> lostMap = request.getLostStorageMap();
        assertEquals(1, lostMap.size());
        assertEquals(StorageList.newBuilder().build(), lostMap.get("MEM"));
    ...
    assertEquals(containedBlockIds.size(), RegisterStreamTestUtils.TIER_BLOCK_TOTAL);
  }
  ...
  public void normalFlow() throws Exception {
    ...
    RegisterStreamer registerStreamer = new RegisterStreamer(asyncClient,
        WORKER_ID, mTierAliases, mCapacityMap, mUsedMap, mBlockMap, RegisterStreamTestUtils.LOST_STORAGE, RegisterStreamTestUtils.EMPTY_CONFIG);
  ...
  public void deleteDuringRegisterStream() throws Exception {
    ...
    prepareBlocksOnWorker(RegisterStreamTestUtils.TIER_CONFIG);
    ...
    BlockMasterSync sync = new BlockMasterSync(
        mBlockWorker, workerId, RegisterStreamTestUtils.NET_ADDRESS_1, mBlockMasterClientPool);
  ...
  private void prepareBlocksOnWorker(String tierConfig) {
    List<String> tierAliases = RegisterStreamTestUtils.getTierAliases(RegisterStreamTestUtils.parseTierConfig(tierConfig));
    // Generate block IDs heuristically
    Map<TierAlias, List<Integer>> tierConfigMap = RegisterStreamTestUtils.parseTierConfig(tierConfig);
