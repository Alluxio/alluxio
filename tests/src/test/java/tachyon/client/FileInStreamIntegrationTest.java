...
public class FileInStreamIntegrationTest {
  ...
  public void readTest1() throws IOException, TachyonException {
    ...
    for (int k = MIN_LEN; k <= MAX_LEN; k += DELTA) {
      for (CreateFileOptions op : getOptionSet()) {
        ...
        FileSystemTestUtils.createByteFile(sTfs, filename, k, op);

        FileInStream is = sTfs.openFile(uri, FileSystemTestUtils.toOpenFileOptions(op));
        byte[] ret = new byte[k];
        int value = is.read();
        int cnt = 0;
        while (value != -1) {
        ...
        is = sTfs.openFile(uri, FileSystemTestUtils.toOpenFileOptions(op));
        ret = new byte[k];
        value = is.read();
        cnt = 0;
        while (value != -1) {
  ...
  public void eofSeekTest() throws IOException, TachyonException {
    ...
    for (CreateFileOptions op : getOptionSet()) {
      ...
      TachyonURI uri = new TachyonURI(filename);
      FileSystemTestUtils.createByteFile(sTfs, filename, length, op);
