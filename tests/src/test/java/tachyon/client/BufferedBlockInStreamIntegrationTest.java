  ...
  public void readTest1() throws IOException, TachyonException {
    ...
    for (int k = MIN_LEN; k <= MAX_LEN; k += DELTA) {
      for (CreateFileOptions op : getOptionSet()) {
        ...
        FileSystemTestUtils.createByteFile(sTfs, path, op, k);

        for (int i = 0; i < 2; i ++) {
          FileInStream is = sTfs.openFile(path, FileSystemTestUtils.toOpenFileOptions(op));
          byte[] ret = new byte[k];
          int value = is.read();
          int cnt = 0;
          while (value != -1) {
  ...
  public void readTest2() throws IOException, TachyonException {
    ...
    for (int k = MIN_LEN; k <= MAX_LEN; k += DELTA) {
      for (CreateFileOptions op : getOptionSet()) {
        ...
        is = sTfs.openFile(path, FileSystemTestUtils.toOpenFileOptions(op));
        ret = new byte[k];
        Assert.assertEquals(k, is.read(ret));
        Assert.assertTrue(BufferUtils.equalIncreasingByteArray(k, ret));
        is.close();
