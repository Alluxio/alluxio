...
# Cross cluster master specific parameters based on ALLUXIO_JAVA_OPTS.
ALLUXIO_CROSS_CLUSTER_MASTER_JAVA_OPTS_DEFAULT=" -Dalluxio.logger.type=${ALLUXIO_CROSS_CLUSTER_MASTER_LOGGER:-CROSS_CLUSTER_MASTER_LOGGER}"
if [[ -n "${ALLUXIO_LOGSERVER_HOSTNAME}" && -n "${ALLUXIO_LOGSERVER_PORT}" ]]; then
    ALLUXIO_CROSS_CLUSTER_MASTER_JAVA_OPTS_DEFAULT+=" -Dalluxio.remote.logger.type=REMOTE_CROSS_CLUSTER_MASTER_LOGGER"
fi
ALLUXIO_CROSS_CLUSTER_MASTER_JAVA_OPTS="${ALLUXIO_CROSS_CLUSTER_MASTER_JAVA_OPTS_DEFAULT} ${ALLUXIO_JAVA_OPTS} ${ALLUXIO_CROSS_CLUSTER_MASTER_JAVA_OPTS}"
