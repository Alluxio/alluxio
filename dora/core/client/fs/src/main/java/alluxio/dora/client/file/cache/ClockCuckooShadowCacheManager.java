...
public class ClockCuckooShadowCacheManager implements ShadowCacheManager {
  ...
  public ClockCuckooShadowCacheManager(AlluxioConfiguration conf) {
    ...
    long expectedInsertions = (long) (Long.highestOneBit(totalSlots) * ConcurrentClockCuckooFilter.DEFAULT_LOAD_FACTOR);
    if (isSizeEncoderEnabled) {
      ...
      mFilter = ConcurrentClockCuckooFilter.create(CacheManagerWithShadowCache.PageIdFunnel.FUNNEL,
          ...
          SlidingWindowType.TIME_BASED, windowMs, ConcurrentClockCuckooFilter.DEFAULT_FPP, ConcurrentClockCuckooFilter.DEFAULT_LOAD_FACTOR,
          Hashing.murmur3_128());
    } else {
      mFilter = ConcurrentClockCuckooFilter.create(CacheManagerWithShadowCache.PageIdFunnel.FUNNEL,
