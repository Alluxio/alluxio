...
public class BlockInStreamTest {
  ...
  public void before() throws Exception {
    ...
    mOptions = new InStreamOptions(
        new URIStatus(new FileInfo().setBlockIds(Collections.singletonList(1L))), mConf,
        mMockContext);
  }
  ...
  public void closeReaderAfterReadingAllData() throws Exception {
    int chunkSize = 512;
    TestDataReader.Factory factory =
        new TestDataReader.Factory(chunkSize, BufferUtils.getIncreasingByteArray(2 * chunkSize));
  ...
  public void createDomainSocketEnabled() throws Exception {
    ...
    PowerMockito
        .when(NettyUtils.isDomainSocketAccessible(ArgumentMatchers.any(WorkerNetAddress.class),
            ArgumentMatchers.any(InstancedConfiguration.class)))
        .thenReturn(true);
    PowerMockito
        .when(NettyUtils.isDomainSocketSupported(ArgumentMatchers.any(WorkerNetAddress.class)))
        .thenReturn(true);
    WorkerNetAddress dataSource = new WorkerNetAddress();
