...
public class UfsUrl {
  ...
  private static final String ALLUXIO_SCHEME_UNSUPPORTED_MSG =
  ...
  private static class Parser {
    ...
    private Parser(String inputUrl) throws IllegalArgumentException {
      ...
      int nextStart = parseScheme(inputUrl);
      nextStart = parseAuthority(inputUrl, nextStart);
      parsePath(inputUrl, nextStart);
    }

    @SuppressFBWarnings(value = "NP_NULL_ON_SOME_PATH",
        justification = "findbugs complains scheme is null "
            + "because that is guarded by Preconditions.")
    private int parseScheme(String inputString)
      ...
      if (firstSlash != -1 && firstColon + 1 != firstSlash) {
        // have colon and slash, but slash is not right behind the colon -> illegal empty scheme
        throw new IllegalArgumentException(String.format("empty scheme: %s", inputString));
      }
      if (firstSlash == -1) {
      ...
      Preconditions.checkArgument(scheme != null && !scheme.isEmpty(),
          "null scheme or empty scheme: %s", inputString);
      Preconditions.checkArgument(!scheme.equalsIgnoreCase(Constants.SCHEME),
          ALLUXIO_SCHEME_UNSUPPORTED_MSG);
      mScheme = scheme;
      return nextStart;
    }

    private int parseAuthority(String inputString, Integer begin) {
    ...
    private void parsePath(String inputUrl, int begin) {
  ...
  public UfsUrl(String scheme, String authority, String path) {
    Preconditions.checkArgument(scheme != null && !scheme.isEmpty(),
        "null scheme or empty scheme: %s", scheme + authority + path);
    Preconditions.checkArgument(!scheme.equalsIgnoreCase(Constants.SCHEME),
        ALLUXIO_SCHEME_UNSUPPORTED_MSG);
  ...
  public boolean isAncestorOf(UfsUrl ufsUrl) {
    ...
    if (curDepth >= ufsUrl.getDepth())  {
      return false;
    }
    List<String> curPathComponents = getPathComponents();
    List<String> targetPathComponents = ufsUrl.getPathComponents();
    for (int i = 0; i < curDepth; i++)  {
      if (!curPathComponents.get(i).equals(targetPathComponents.get(i))) {
        return false;
      }
    }
  ...
  public UfsUrl join(String suffix) {
    ...
    List<String> notEmptyComponentsList = new ArrayList<>(suffixArray.length);
    for (int i = 0; i < suffixArray.length; i++) {
      if (!suffixArray[i].isEmpty()) {
        notEmptyComponentsList.add(suffixArray[i]);
      }
    }
    List<String> pathComponents = mProto.getPathComponentsList();
    return new UfsUrl(UfsUrlMessage.newBuilder()
        ...
        .addAllPathComponents(notEmptyComponentsList).build());
  }
}
