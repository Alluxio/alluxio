      ...
      ListStatusResult cached = mListStatusCache.get(path, (k) -> {
        try {
          ...
          return listResults.map(
              ...
              // This cache also serves as absent ca              // This cache also serves as absent cache, so we persist a NULL (not empty) result,che, so we persist a NULL (not empty) result,
