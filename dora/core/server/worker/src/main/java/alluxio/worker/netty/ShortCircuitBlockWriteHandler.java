          ...
          public Void call() throws Exception {
            if (request.getOnlyReserveSpace()) {
              ...
              if (mSessionId == INVALID_SESSION_ID) {
                ...
                String mediumType = "";
                if (Configuration.global().isSet(PropertyKey.USER_FILE_TARGET_MEDIA)) {
                  mediumType = Configuration.global().getString(PropertyKey.USER_FILE_TARGET_MEDIA);
                }
                ...
                String path = mBlockWorker.createBlock(mSessionId, request.getBlockId(),
                    request.getTier(), createBlockOptions);
        ...
        RpcUtils.nettyRPCAndLog(LOG, new RpcUtils.NettyRPCCallable<Void>() {
          ...
          public Void call() throws Exception {
            if (request.getCancel()) {
              ...
              mBlockWorker.commitBlock(mSessionId, request.getBlockId(), false);
            }
            mSessionId = INVALID_SESSION_ID;
            ctx.writeAndFlush(RPCProtoMessage.createOkResponse(null));
            return null;
