...
public class InodeSyncStream {
   ...
   *
   * The root path should be already locked with {@link InodeTree.LockPattern#WRITE_EDGE} unless the user is
   * only planning on loading metadata. The desired pattern should always be
   * {@link InodeTree.LockPattern#READ}.
   *
  ...
  private SyncStatus syncInternal() throws
    ...
    while (!mPendingPaths.isEmpty() || !mSyncPathJobs.isEmpty()) {
      ...
      // successfully
      while (true) {
        Future<SyncCheck.SyncResult> job = mSyncPathJobs.peek();
        ...
        try {
          ...
          SyncCheck.SyncResult result = job.get();
      ...
      for (int i = 0; i < submissions; i++) {
        ...
        Future<SyncCheck.SyncResult> job =
      ...
      Future<SyncCheck.SyncResult> oldestJob = mSyncPathJobs.peek();
   ...
   */
  private SyncCheck.SyncResult processSyncPath(AlluxioURI path, RpcContext rpcContext)
  ...
  private SyncCheck.SyncResult processSyncPathInternal(AlluxioURI path, RpcContext rpcContext)
      throws InvalidPathException {
    if (path == null) {
    ...
    if (mForceSync || forceSync) {
      scheme = new LockingScheme(path, InodeTree.LockPattern.READ, true);
    } else {
      scheme = new LockingScheme(path, InodeTree.LockPattern.READ, mSyncOptions,
  ...
  private void syncExistingInodeMetadata(
      ...
      InterruptedException {
    if (inodePath.getLockPattern() != InodeTree.LockPattern.WRITE_EDGE && !mLoadOnly) {
