...
public class DoraLoadJob extends AbstractJob<DoraLoadJob.DoraLoadTask> {
  ...
  private static final double FAILURE_RATIO_THRESHOLD = Configuration.getDouble(
      PropertyKey.SCHEDULER_DORA_LOAD_JOB_TOTAL_FAILURE_RATIO_THRESHOLD);
  private static final int FAILURE_COUNT_THRESHOLD = Configuration.getInt(
      PropertyKey.SCHEDULER_DORA_LOAD_JOB_TOTAL_FAILURE_COUNT_THRESHOLD);
  ...
  private final Supplier<Integer> mRetryThresholds = Suppliers.memoizeWithExpiration(
      () -> Configuration.getInt(PropertyKey.SCHEDULER_DORA_LOAD_JOB_RETRIES),
      1, TimeUnit.MINUTES);
      ...
      if (response.getStatus() != TaskStatus.SUCCESS) {
        ...
        for (LoadFileFailure failure : response.getFailuresList()) {
          ...
          if (!isHealthy() || !failure.getRetryable()
              || mRetryCount.getOrDefault(fullUfsPath , 0) >= mRetryThresholds.get()
              || !addFilesToRetry(
