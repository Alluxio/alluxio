...
package alluxio.dora.master.file.replication;
...
import alluxio.dora.replicate.DefaultReplicationHandler;
import alluxio.dora.replicate.ReplicationHandler;
  ...
  private void checkMisreplicated(Set<Long> inodes, ReplicationHandler handler)
    ...
    for (long inodeId : inodes) {
      ...
      try (LockedInodePath inodePath =
               mInodeTree.lockFullInodePath(inodeId, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)
      ) {
  ...
  private Set<Long> check(Set<Long> inodes, ReplicationHandler handler, Mode mode)
    ...
    for (long inodeId : inodes) {
      ...
      try (LockedInodePath inodePath = mInodeTree.lockFullInodePath(
          inodeId, InodeTree.LockPattern.READ, NoopJournalContext.INSTANCE)
      ) {
        InodeFile file = inodePath.getInodeFile();
        for (long blockId : file.getBlockIds()) {
          BlockInfo blockInfo = null;
