...
public final class SchedulerTest {
  ...
  public void testScheduling() throws Exception {
    ...
    List<FileInfo> fileInfos = LoadTestUtils.generateRandomFileInfo(100, 50, 64 * Constants.MB);
    when(fsMaster.listStatus(any(), any()))
        .thenReturn(fileInfos)
        .thenReturn(LoadTestUtils.fileWithBlockLocations(fileInfos, 0.95))
        .thenReturn(LoadTestUtils.fileWithBlockLocations(fileInfos, 1.1));
  ...
  private ListenableFuture<LoadResponse> buildResponseFuture(LoadRequest request,
    ...
    if (iteration.get() == exceptionRequestIteration) {
      ...
      status = LoadTestUtils.generateRandomBlockStatus(request.getBlocksList(), 1);
    }
    else {
      status = LoadTestUtils.generateRandomBlockStatus(request.getBlocksList(), 0.01);
    }
  ...
  public void testSchedulingFullCapacity() throws Exception {
    ...
    List<FileInfo> fileInfos = LoadTestUtils.generateRandomFileInfo(2000, 50, 64 * Constants.MB);
    when(fsMaster.listStatus(any(), any()))
        .thenReturn(fileInfos);
  ...
  public void testSchedulingWithException() throws Exception {
    ...
    when(fileSystemContext.acquireBlockWorkerClient(any())).thenReturn(blockWorkerClientResource);
    when(blockWorkerClientResource.get()).thenReturn(blockWorkerClient);
    List<FileInfo> fileInfos = LoadTestUtils.generateRandomFileInfo(100, 10, 64 * Constants.MB);
    when(fsMaster.listStatus(any(), any()))
