...
public final class WorkerBenchParameters extends FileSystemParameters {
  ...
  @Parameter(names = {"--random-max-length"},
      description = "The random max length upper bound." + "As this InputStream.read() only accept read offset and length as Integer, so this max length must smaller than 2.1GB")
