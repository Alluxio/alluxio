...
public class TOSInputStream extends MultiRangeObjectInputStream {
   ...
   */
  TOSInputStream(String bucketName, String key, TOSV2 client,
  ...
  TOSInputStream(String bucketName, String key, TOSV2 client, long position,
    ...
    HeadObjectV2Output output = mTosClient.headObject(input);
    mContentLength = output.getContentLength();
  }
  ...
  protected InputStream createStream(long startPos, long endPos)
    ...
    GetObjectV2Input input = new GetObjectV2Input().setBucket(mBucketName)
        .setKey(mKey).setOptions(options);
    // TOS returns entire object if we read past the end
    TosServerException lastException = null;
    String errorMessage = String.format("Failed to open key: %s bucket: %s", mKey, mBucketName);
    while (mRetryPolicy.attempt()) {
      try {
        GetObjectV2Output output = mTosClient.getObject(input);
        return new BufferedInputStream(output.getContent());
