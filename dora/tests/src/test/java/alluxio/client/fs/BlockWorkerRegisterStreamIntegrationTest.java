...
public class BlockWorkerRegisterStreamIntegrationTest {
  ...
  public void requestsForEmptyWorker() throws Exception {
    ...
    Assert.assertEquals(RegisterStreamTestUtils.MEM_USAGE_EMPTY, request.getUsedBytesOnTiersMap());
    Assert.assertEquals(RegisterStreamTestUtils.MEM_CAPACITY, request.getTotalBytesOnTiersMap());
  ...
  public void requestsForWorker() throws Exception {
    ...
    for (int i = 0; i < expectedBatchCount; i++) {
      ...
      if (i == 0) {
        Assert.assertEquals(RegisterStreamTestUtils.USAGE_MAP, request.getUsedBytesOnTiersMap());
        Assert.assertEquals(RegisterStreamTestUtils.CAPACITY_MAP, request.getTotalBytesOnTiersMap());
        Map<String, StorageList> lostMap = request.getLostStorageMap();
        assertEquals(1, lostMap.size());
        assertEquals(StorageList.newBuilder().build(), lostMap.get("MEM"));
    ...
    Assert.assertEquals(containedBlockIds.size(), RegisterStreamTestUtils.TIER_BLOCK_TOTAL);
  }
