...
public class FuseFileSystemDataTest extends AbstractFuseFileSystemTest {
  ...
  public void createCloseDifferentThread() throws InterruptedException, ExecutionException {
    ...
    mFileInfo.get().flags.set(O_WRONLY.intValue());
    assertEquals(0, mFuseFs.create(path, DEFAULT_MODE.toShort(), mFileInfo.get()));
    ...
    assertEquals(0, mFuseFs.open(path, mFileInfo.get()));
  ...
  public void writeEmptyNonExisting() {
    ...
    createOpenTest(createOrOpenOperation -> {
      ...
      assertEquals(0, mFuseFs.release(path, mFileInfo.get()));
      assertEquals(0, mFuseFs.getattr(path, mFileStat));
      assertEquals(0, mFileStat.st_size.longValue());
  ...
  public void overwriteEmpty() {
    ...
    createOpenTest(createOrOpenOperation -> {
      ...
      assertEquals(DEFAULT_FILE_LEN,
          mFuseFs.write(path, buffer, DEFAULT_FILE_LEN, 0, mFileInfo.get()));
      assertEquals(0, mFuseFs.release(path, mFileInfo.get()));
      assertEquals(0, mFuseFs.getattr(path, mFileStat));
      assertEquals(DEFAULT_FILE_LEN, mFileStat.st_size.intValue());
      assertEquals(0, mFuseFs.unlink(path));
    }, true, path);
  }
  ...
  public void failedToOverwrite() {
    ...
    createOpenTest(createOrOpenOperation -> {
      ...
      ByteBuffer buffer = BufferUtils.getIncreasingByteBuffer(DEFAULT_FILE_LEN);
      assertEquals(-ErrorCodes.EEXIST(),
          mFuseFs.write(path, buffer, DEFAULT_FILE_LEN, 0, mFileInfo.get()));
      assertEquals(0, mFuseFs.release(path, mFileInfo.get()));
      assertEquals(0, mFuseFs.unlink(path));
    }, true, path);
  ...
  public void truncateSame() {
    ...
    createOpenTest(createOrOpenOperation -> {
      ...
      assertEquals(0, mFuseFs.truncate(path, DEFAULT_FILE_LEN));
      assertEquals(0,
          mFuseFs.getattr(path, mFileStat));
      assertEquals(DEFAULT_FILE_LEN, mFileStat.st_size.intValue());
      assertEquals(0, mFuseFs.release(path, mFileInfo.get()));
  ...
  public void truncateMiddle() {
    ...
    createOpenTest(createOrOpenOperation -> {
      ...
      try {
        Assert.assertEquals(0, (int) createOrOpenOperation.apply(0));
        assertEquals(-ErrorCodes.EOPNOTSUPP(), mFuseFs.truncate(path, DEFAULT_FILE_LEN / 2));
      } catch (Exception e) {
        throw new RuntimeException(e);
      } finally {
    ...
    createOpenTest(createOrOpenOperation -> {
      try {
        ...
        assertEquals(0,
            mFuseFs.getattr(path, mFileStat));
        assertEquals(DEFAULT_FILE_LEN, mFileStat.st_size.intValue());
        assertEquals(0, mFuseFs.truncate(path, DEFAULT_FILE_LEN * 2));
        assertEquals(0,
            mFuseFs.getattr(path, mFileStat));
        assertEquals(DEFAULT_FILE_LEN * 2, mFileStat.st_size.intValue());
        ...
        assertEquals(0,
            mFuseFs.getattr(path, mFileStat));
        assertEquals(DEFAULT_FILE_LEN * 3, mFileStat.st_size.intValue());
      } catch (Exception e) {
        throw new RuntimeException(e);
      } finally {
        mFuseFs.release(path, mFileInfo.get());
  ...
  public void randomWrite() {
    ...
    createOpenTest(createOrOpenOperation -> {
      try {
        Assert.assertEquals(0, (int) createOrOpenOperation.apply(0));
        ByteBuffer buffer = BufferUtils.getIncreasingByteBuffer(DEFAULT_FILE_LEN);
        assertEquals(-ErrorCodes.EOPNOTSUPP(),
  ...
  public void openReadNonExisting() {
    ...
    mFileInfo.get().flags.set(O_RDONLY.intValue());
    assertEquals(-ErrorCodes.ENOENT(), mFuseFs.open(path, mFileInfo.get()));
  }
    ...
    createOpenTest(createOrOpenOperation -> {
      try {
        ...
        assertEquals(DEFAULT_FILE_LEN,
            mFuseFs.write(path, buffer, DEFAULT_FILE_LEN, 0, mFileInfo.get()));
        mFileInfo.get().flags.set(O_RDONLY.intValue());
        assertEquals(-ErrorCodes.ETIME(), mFuseFs.open(path, mFileInfo.get()));
      } catch (Exception e) {
        throw new RuntimeException(e);
      } finally {
        assertEquals(0, mFuseFs.release(path, mFileInfo.get()));
  ...
  public void writeThenRead() {
    ...
    mFileInfo.get().flags.set(O_WRONLY.intValue());
    assertEquals(0, mFuseFs.create(path,
  ...
  public void getAttrWhenWriting() {
    ...
    assertEquals(0,
        mFuseFs.getattr(path, mFileStat));
    assertEquals(DEFAULT_FILE_LEN, mFileStat.st_size.intValue());
    assertEquals(AlluxioFuseUtils.getSystemUid(), mFileStat.st_uid.get());
    assertEquals(AlluxioFuseUtils.getSystemGid(), mFileStat.st_gid.get());
    assertEquals(AlluxioFuseUtils.getSystemGid(), mFileStat.st_gid.get());
