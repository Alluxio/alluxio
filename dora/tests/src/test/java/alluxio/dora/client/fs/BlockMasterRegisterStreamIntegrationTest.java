...
public class BlockMasterRegisterStreamIntegrationTest {
  ...
  public void registerEmptyWorkerStream() throws Exception {
    long workerId = mBlockMaster.getWorkerId(RegisterStreamTestUtils.NET_ADDRESS_1);
    ...
    Assert.assertEquals(RegisterStreamTestUtils.MEM_CAPACITY_BYTES, worker.getCapacityBytes());
    Assert.assertEquals(RegisterStreamTestUtils.MEM_CAPACITY_BYTES, worker.getAvailableBytes());
  ...
  public void registerWorkerStream() throws Exception {
    ...
    assertEquals(0, errorQueue.size());
    MasterWorkerInfo worker = mBlockMaster.getWorker(workerId);
    Assert.assertEquals(RegisterStreamTestUtils.TIER_BLOCK_TOTAL, worker.getBlockCount());
    assertEquals(0, worker.getToRemoveBlockCount());
    assertEquals(1, mBlockMaster.getWorkerCount());
  ...
  public void registerLostWorker() throws Exception {
    ...
    Assert.assertEquals(RegisterStreamTestUtils.TIER_BLOCK_TOTAL, updatedWorker.getBlockCount());
  ...
  public void registerExistingWorkerBlocksLost() throws Exception {
    ...
    List<String> tierAliases = getTierAliases(RegisterStreamTestUtils.parseTierConfig(RegisterStreamTestUtils.TIER_CONFIG));
    Map<BlockStoreLocation, List<Long>> blockMap =
        RpcBenchPreparationUtils.generateBlockIdOnTiers(RegisterStreamTestUtils.parseTierConfig(RegisterStreamTestUtils.TIER_CONFIG));
    ...
    RegisterStreamer newRegisterStreamer = new RegisterStreamer(null,
        workerId, tierAliases, RegisterStreamTestUtils.CAPACITY_MAP, RegisterStreamTestUtils.USAGE_MAP, blockMap, RegisterStreamTestUtils.LOST_STORAGE, RegisterStreamTestUtils.EMPTY_CONFIG);
    List<RegisterWorkerPRequest> newRequestChunks = ImmutableList.copyOf(newRegisterStreamer);
    int newExpectedBatchCount =
        (int) Math.ceil((RegisterStreamTestUtils.TIER_BLOCK_TOTAL - lostBlocks.size()) / (double) RegisterStreamTestUtils.BATCH_SIZE);
    ...
    assertEquals(1, mBlockMaster.getWorkerCount());
    MasterWorkerInfo updatedWorker = mBlockMaster.getWorker(workerId);
    Assert.assertEquals(RegisterStreamTestUtils.TIER_BLOCK_TOTAL - lostBlocks.size(), updatedWorker.getBlockCount());
  ...
  public void registerExistingWorkerBlocksAdded() throws Exception {
    ...
    RegisterStreamer newRegisterStreamer = new RegisterStreamer(null,
        workerId, tierAliases, capacityMap, usedMap, blockMap, RegisterStreamTestUtils.LOST_STORAGE, RegisterStreamTestUtils.EMPTY_CONFIG);
    List<RegisterWorkerPRequest> newRequestChunks = ImmutableList.copyOf(newRegisterStreamer);
    int newExpectedBatchCount =
        (int) Math.ceil((RegisterStreamTestUtils.TIER_BLOCK_TOTAL + addedBlocks.size()) / (double) RegisterStreamTestUtils.BATCH_SIZE);
    assertEquals(newExpectedBatchCount, newRequestChunks.size());
    ...
    assertEquals(1, mBlockMaster.getWorkerCount());
    MasterWorkerInfo updatedWorker = mBlockMaster.getWorker(workerId);
    Assert.assertEquals(RegisterStreamTestUtils.TIER_BLOCK_TOTAL + addedBlocks.size(), updatedWorker.getBlockCount());
    assertEquals(0, updatedWorker.getToRemoveBlockCount());
  ...
  public void hangingWorkerSessionRecycled() throws Exception {
    ...
    verifyWorkerCanReregister(workerId, requestChunks, RegisterStreamTestUtils.TIER_BLOCK_TOTAL);
    verifyWorkerWritable(workerId);
  }
  ...
  public void workerRegisterStartThrowsError() throws Exception {
    ...
    long workerId = brokenBlockMaster.getWorkerId(RegisterStreamTestUtils.NET_ADDRESS_1);
    Queue<Throwable> errorQueue = new ConcurrentLinkedQueue<>();
  ...
  public void reregisterWithDelete() throws Exception {
    ...
    MasterWorkerInfo worker = mBlockMaster.getWorker(workerId);
    assertEquals(1, mBlockMaster.getWorkerCount());
    Assert.assertEquals(RegisterStreamTestUtils.TIER_BLOCK_TOTAL - 1, worker.getBlockCount());
  ...
  private Command sendHeartbeatToMaster(long workerId) {
    return mBlockMaster.workerHeartbeat(
        workerId,
        RegisterStreamTestUtils.CAPACITY_MAP,
        RegisterStreamTestUtils.USAGE_MAP,
