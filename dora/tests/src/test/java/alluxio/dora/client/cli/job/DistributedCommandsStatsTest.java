...
public class DistributedCommandsStatsTest extends JobShellTest {
  ...
  public void before() throws Exception {
    ...
    AbstractFileSystemShellTest.sFileSystem.mount(new AlluxioURI("/mnt/"), new AlluxioURI("sleep://" + mLocalUfsPath));
  }

  @Test
  public void testCompleteStats() throws Exception {
    final int length = 10;
    FileSystemTestUtils.createByteFile(AbstractFileSystemShellTest.sFileSystem, "/test", WritePType.THROUGH, length);

    long jobId = AbstractFileSystemShellTest.sJobMaster.run(new LoadConfig("/test", 1, Collections.EMPTY_SET,
            Collections.EMPTY_SET, Collections.EMPTY_SET, Collections.EMPTY_SET, false));

    JobTestUtils
            .waitForJobStatus(AbstractFileSystemShellTest.sJobMaster, jobId, Sets.newHashSet(Status.COMPLETED), TEST_TIMEOUT);

    AbstractFileSystemShellTest.sJobShell.run("stat", "-v", Long.toString(jobId));
  ...
  public void testDistributedLoadCancelStats() throws Exception {
    FileSystemTestUtils.createByteFile(AbstractFileSystemShellTest.sFileSystem,  "/mnt/testFileNew",
            WritePType.THROUGH, 10);

    long jobId = AbstractFileSystemShellTest.sJobMaster.run(new LoadConfig("/mnt/testFileNew",
    ...
    JobTestUtils
        .waitForJobStatus(AbstractFileSystemShellTest.sJobMaster, jobId, FINISH_STATUS, TEST_TIMEOUT);
  ...
  public void testDistributedCpCancelStats() throws Exception {
    FileSystemTestUtils.createByteFile(AbstractFileSystemShellTest.sFileSystem,  "/mnt/testFileSource",
            WritePType.THROUGH, 10);

    long jobId = AbstractFileSystemShellTest.sJobMaster.run(new MigrateConfig(
  ...
  public void testAsyncPersistCancelStats() throws Exception {
    FileSystemTestUtils.createByteFile(AbstractFileSystemShellTest.sFileSystem,  "/mnt/testFile",
            WritePType.MUST_CACHE, 10000);

    long jobId = AbstractFileSystemShellTest.sJobMaster.run(new PersistConfig("/mnt/testFile",
