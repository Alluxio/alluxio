...
public class JournalIntegrationTest {
  ...
  public void AddBlockTest() throws Exception {
    TachyonURI uri = new TachyonURI(PathUtils.concatPath(mMountPoint, "xyz"));
  ...
  private void deleteFsMasterJournalLogs() throws IOException {
    ...
    Journal journal =
        new ReadWriteJournal(PathUtils.concatPath(journalFolder,
  ...
  private void AddBlockTestUtil(FileInfo fileInfo) throws IOException, InvalidPathException,
    ...
    long xyzId = fsMaster.getFileId(new TachyonURI(PathUtils.concatPath(mMountPoint, "xyz")));
  ...
  public void CompletedEditLogDeletionTest() throws Exception {
    for (int i = 0; i < 124; i ++) {
      mTfs.getOutStream(new TachyonURI(PathUtils.concatPath(mMountPoint, "a" + i)),
          new ClientOptions.Builder(mMasterTachyonConf).setBlockSize((i + 10) / 10 * 64).build())
          .close();
    }
    mLocalTachyonCluster.stopTFS();
  ...
  private void DeleteTestUtil()
    ...
    for (int i = 0; i < 5; i ++) {
      for (int j = 0; j < 5; j ++) {
        Assert.assertTrue(fsMaster.getFileId(
            new TachyonURI(PathUtils.concatPath(mMountPoint, "i" + i, "j" + j))) != -1);
      }
    }
    fsMaster.stop();
  }
    ...
    for (int i = 0; i < 10; i ++) {
      ...
      for (int j = 0; j < 10; j ++) {
        ClientOptions option =
            new ClientOptions.Builder(mMasterTachyonConf).setBlockSize((i + j + 1) * 64).build();
        mTfs.getOutStream(
            new TachyonURI(PathUtils.concatPath(mMountPoint, "i" + i, "j" + j)), option).close();
      }
    }
    mLocalTachyonCluster.stopTFS();
    FileFolderUtil();
  ...
  public void FileTest() throws Exception {
    ...
    TachyonURI filePath = new TachyonURI(PathUtils.concatPath(mMountPoint, "xyz"));
  ...
  private void FileTestUtil(FileInfo fileInfo) throws IOException, InvalidPathException,
    ...
    Assert.assertTrue(rootId != -1);
    Assert.assertEquals(1, fsMaster.getFileInfoList(rootId).size());
    long fileId = fsMaster.getFileId(new TachyonURI(PathUtils.concatPath(mMountPoint, "xyz")));
  ...
  public void PinTest() throws Exception {
    mTfs.mkdirs(new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder")));
    TachyonFile folder = mTfs.open(new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder")));
    mTfs.setPin(folder, true);

    TachyonURI file0Path = new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder", "file0"));
    ...
    TachyonURI file1Path = new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder", "file1"));
  ...
  private void PinTestUtil(FileInfo folder, FileInfo file0, FileInfo file1) throws IOException,
      InvalidPathException, FileDoesNotExistException {
    FileSystemMaster fsMaster = createFsMasterFromJournal();

    FileInfo info = fsMaster.getFileInfo(fsMaster.getFileId(
        new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder"))));
    Assert.assertEquals(folder, info);
    Assert.assertTrue(info.isPinned);

    info = fsMaster.getFileInfo(fsMaster.getFileId(
        new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder", "file0"))));
    Assert.assertEquals(file0, info);
    Assert.assertFalse(info.isPinned);

    info = fsMaster.getFileInfo(fsMaster.getFileId(
        new TachyonURI(PathUtils.concatPath(mMountPoint, "myFolder", "file1"))));
  ...
  public void FolderTest() throws Exception {
    TachyonURI folderPath = new TachyonURI(PathUtils.concatPath(mMountPoint, "xyz"));
  ...
  public void ManyFileTest() throws Exception {
    for (int i = 0; i < 10; i ++) {
      ClientOptions option =
          new ClientOptions.Builder(mMasterTachyonConf).setBlockSize((i + 1) * 64).build();
      mTfs.getOutStream(new TachyonURI(PathUtils.concatPath(mMountPoint, "a" + i)), option).close();
    }
    mLocalTachyonCluster.stopTFS();
    ManyFileTestUtil();
    deleteFsMasterJournalLogs();
  ...
  private void ManyFileTestUtil() throws IOException, InvalidPathException,
    ...
    for (int k = 0; k < 10; k ++) {
      Assert.assertTrue(fsMaster.getFileId(
          new TachyonURI(PathUtils.concatPath(mMountPoint, "a" + k))) != -1);
    }
    fsMaster.stop();
  }
  ...
  public void MultiEditLogTest() throws Exception {
    for (int i = 0; i < 124; i ++) {
      ClientOptions op =
          new ClientOptions.Builder(mMasterTachyonConf).setBlockSize((i + 10) / 10 * 64).build();
      mTfs.getOutStream(new TachyonURI(PathUtils.concatPath(mMountPoint, "a" + i)), op);
    }
    mLocalTachyonCluster.stopTFS();
    MultiEditLogTestUtil();
    deleteFsMasterJournalLogs();
  ...
  private void RenameTestUtil()
    ...
    for (int i = 0; i < 10; i ++) {
      for (int j = 0; j < 10; j ++) {
        Assert.assertTrue(fsMaster.getFileId(
            new TachyonURI(PathUtils.concatPath(mMountPoint, "ii" + i, "jj" + j))) != -1);
      }
    }
    fsMaster.stop();
  }
