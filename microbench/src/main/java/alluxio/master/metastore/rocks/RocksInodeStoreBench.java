...
public class RocksInodeStoreBench {
  ...
  @Threads(1)
  @Benchmark
  public long OneRockGetMutableBench(RockState rs) {
    long counter = 0;
    MutableInode file = null;
    for (long i = rs.mInodeCount; i > 0; i--) {
      rs.mRock.getMutable(i);
  ...
  @Threads(1)
  @Benchmark
  public long OneRockGetChildIdBench(RockState rs) {
    long counter = 0;
    Long tmp = Long.valueOf(0);
    Long p = Long.valueOf(0);
    for (long i = rs.mInodeCount; i > 0; i--) {
  ...
  @Threads(1)
  @Benchmark
  public long OneRockGetChildIdsBench(RockGetChildIdsState rs) {
    long counter = 0;
    for (long i = rs.mTreeNumber; i > 0; i--) {
      try (CloseableIterator<Long> iter = rs.mRock.getChildIds(i, ReadOption.defaults())) {
        while (iter.hasNext()) {
  ...
  @Threads(1)
  @Benchmark
  public long OneRockAddChildIdBench(RockAddChildState rs) {
    long counter = 0;
    long limit = rs.mInodeCount;
    for (long i = 1; i <= limit; i++) {
      rs.mRock.addChild(0, "test" + i, i);
